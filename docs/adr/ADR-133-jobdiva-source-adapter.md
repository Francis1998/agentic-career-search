# ADR-133: JobDiva Public Careers Source Adapter
...
JobDiva is an applicant tracking / recruiting platform whose public career sites
commonly host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`,
`/careers/{id}`, `/careers/job/{id}`, and `/jd/{id}`.

The existing HTML adapters (Tribepad, Vincere, RecruitCRM) already use
deterministic URL-shape matching instead of brittle CSS-only scraping. JobDiva
fits that pattern. Keeping discovery on the public HTML board avoids depending on
authenticated JobDiva APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `JobDivaAdapter` (`source_type = "jobdiva"`) that:

1. Recognises `/jobs/{id}` plural paths.
2. Accepts `/job/{id}` singular paths.
3. Accepts `/careers/{id}` and `/careers/job/{id}` careers paths.
4. Accepts `/jd/{id}` jd paths.
...
- JobDiva-hosted public boards are covered without authenticated APIs or
  headless browsers.
