# ADR-125: TalentLyft Public Careers Source Adapter
...
TalentLyft is an applicant tracking platform whose public career sites commonly
...
URL-shape matching instead of brittle CSS-only scraping. TalentLyft fits that
pattern. Keeping discovery on the public HTML board avoids depending on
authenticated TalentLyft APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `TalentLyftAdapter` (`source_type = "talentlyft"`) that:
...
- TalentLyft-hosted public boards are covered without authenticated APIs or
  headless browsers.
