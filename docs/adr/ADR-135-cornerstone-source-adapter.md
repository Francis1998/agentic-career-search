# ADR-135: Cornerstone Public Careers Source Adapter
...
Cornerstone OnDemand is an applicant tracking / recruiting platform whose public career sites
...
deterministic URL-shape matching instead of brittle CSS-only scraping. Cornerstone
fits that pattern. Keeping discovery on the public HTML board avoids depending on
authenticated Cornerstone APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `CornerstoneAdapter` (`source_type = "cornerstone"`) that:
...
- Cornerstone-hosted public boards are covered without authenticated APIs or
  headless browsers.
