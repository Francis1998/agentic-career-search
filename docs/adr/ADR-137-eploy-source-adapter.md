# ADR-137: Eploy Public Careers Source Adapter
...
eploy is an applicant tracking / recruiting platform whose public career sites
commonly host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`,
`/careers/{id}`, `/careers/job/{id}`, and `/role/{id}`.

The existing HTML adapters (Tribepad, Vincere, RecruitCRM) already use
deterministic URL-shape matching instead of brittle CSS-only scraping. Eploy
fits that pattern. Keeping discovery on the public HTML board avoids depending on
authenticated Eploy APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `EployAdapter` (`source_type = "eploy"`) that:

1. Recognises `/jobs/{id}` plural paths.
2. Accepts `/job/{id}` singular paths.
3. Accepts `/careers/{id}` and `/careers/job/{id}` careers paths.
4. Accepts `/role/{id}` role paths.
...
- Eploy-hosted public boards are covered without authenticated APIs or
  headless browsers.
