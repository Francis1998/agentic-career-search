# ADR-118: HiBob Public Careers Source Adapter
...
HiBob (Bob) is an HRIS/ATS popular with distributed and mid-market teams. Its
...
deterministic URL-shape matching instead of brittle CSS-only scraping. HiBob
fits that pattern. Keeping discovery on the public HTML board avoids depending
on authenticated HiBob APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `HibobAdapter` (`source_type = "hibob"`) that:
...
- HiBob-hosted public boards are covered without authenticated APIs or
  headless browsers.
