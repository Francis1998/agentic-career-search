# ADR-124: Ceipal Public Careers Source Adapter
...
Ceipal is an applicant tracking platform whose public career sites commonly
...
URL-shape matching instead of brittle CSS-only scraping. Ceipal fits that
pattern. Keeping discovery on the public HTML board avoids depending on
authenticated Ceipal APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `CeipalAdapter` (`source_type = "ceipal"`) that:

1. Recognises `/jobs/{id}` plural paths.
2. Accepts `/job/{id}` singular paths.
3. Accepts `/careers/{id}` and `/careers/job/{id}` careers paths.
4. Accepts `/requisitions/{id}` requisitions paths.
...
- Ceipal-hosted public boards are covered without authenticated APIs or
  headless browsers.
