# ADR-130: Vincere Public Careers Source Adapter
...
Vincere is an applicant tracking / recruiting platform whose public career sites
commonly host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`,
`/careers/{id}`, `/careers/job/{id}`, and `/job-detail/{id}`.

The existing HTML adapters (CareerPlug, TrackerRMS, ApplicantStack) already use
deterministic URL-shape matching instead of brittle CSS-only scraping. Vincere
fits that pattern. Keeping discovery on the public HTML board avoids depending on
authenticated Vincere APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `VincereAdapter` (`source_type = "vincere"`) that:

1. Recognises `/jobs/{id}` plural paths.
2. Accepts `/job/{id}` singular paths.
3. Accepts `/careers/{id}` and `/careers/job/{id}` careers paths.
4. Accepts `/job-detail/{id}` job detail paths.
...
- Vincere-hosted public boards are covered without authenticated APIs or
  headless browsers.
