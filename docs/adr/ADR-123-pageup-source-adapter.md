# ADR-123: PageUp Public Careers Source Adapter
...
PageUp is an applicant tracking platform whose public career sites commonly
host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`,
`/careers/{id}`, `/careers/job/{id}`, and `/opportunities/{id}`.

The existing HTML adapters (Jobylon, Dover, JobAdder) already use deterministic
URL-shape matching instead of brittle CSS-only scraping. PageUp fits that
pattern. Keeping discovery on the public HTML board avoids depending on
authenticated PageUp APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `PageUpAdapter` (`source_type = "pageup"`) that:
...
- PageUp-hosted public boards are covered without authenticated APIs or
  headless browsers.
