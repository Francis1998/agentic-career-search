# ADR-120: JobAdder Public Careers Source Adapter
...
JobAdder is a widely used applicant tracking platform. Its public career sites
commonly host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`,
`/careers/{id}`, `/careers/job/{id}`, and `/position/{id}`.

The existing HTML adapters (Factorial, Paylocity) already use deterministic
URL-shape matching instead of brittle CSS-only scraping. JobAdder fits that
pattern. Keeping discovery on the public HTML board avoids depending on
authenticated JobAdder APIs and stays aligned with the shared adapter contract
in ADR-077.

## Decision

Add a `JobAdderAdapter` (`source_type = "jobadder"`) that:
...
- JobAdder-hosted public boards are covered without authenticated APIs or
  headless browsers.
