# ADR-145: Radancy Public Careers Source Adapter
...
Radancy is an employer-brand / ATS careers platform whose public
boards commonly host listing pages with detail pages under `/jobs/{id}`, `/job/{id}`, `/search/job/{id}`, `/careers/{id}`, `/careers/job/{id}`.

The existing HTML adapters (ADP, Paradox, Catsone) use deterministic URL-shape
matching instead of brittle CSS-only scraping. Radancy fits that
...
Add a `RadancyAdapter` (`source_type = "radancy"`) that:
...
- Radancy-hosted public recruiting boards are covered without
