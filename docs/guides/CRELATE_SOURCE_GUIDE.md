# Crelate Source Guide

![Crelate discovery flow](../../assets/demo/crelate-source.gif)

Use this guide when wiring a public Crelate careers board into
...
Crelate (`*.crelate.com`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/opportunity/{id}` detail paths. This adapter mirrors the Tribepad URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-crelate",
    "source_type": "crelate",
    "base_url": "https://acme.crelate.com/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/opportunity`),
...
- Public careers pages only - no authenticated Crelate APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-132 for the design decision.
