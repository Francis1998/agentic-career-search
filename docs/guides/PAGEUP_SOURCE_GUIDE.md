...
![PageUp discovery flow](../../assets/demo/pageup-source.gif)

Use this guide when wiring a public PageUp careers board into
...
PageUp (`careers.pageuppeople.com`) is an ATS used by employers that publish public
careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/opportunities/{id}` detail paths. This adapter mirrors the Jobylon URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-pageup",
    "source_type": "pageup",
    "base_url": "https://careers.pageuppeople.com/acme/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/opportunities`),
...
- Public careers pages only - no authenticated PageUp APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-123 for the design decision.
