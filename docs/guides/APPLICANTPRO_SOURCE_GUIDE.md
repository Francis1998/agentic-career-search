# ApplicantPro Source Guide

![ApplicantPro discovery flow](../../assets/demo/applicantpro-source.gif)

Use this guide when wiring a public ApplicantPro careers board into
...
ApplicantPro (`*.applicantpro.com`) is an ATS / recruiting platform used by
employers that publish public careers pages. Boards commonly expose each job as
an anchor under `/jobs/{id}`, `/job/{id}`, `/careers/{id}`,
`/careers/job/{id}`, or `/openings/{id}` detail paths. This adapter mirrors the
Beamery URL-shape approach used for other HTML careers sources.
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-applicantpro",
    "source_type": "applicantpro",
    "base_url": "https://acme.applicantpro.com/jobs/"
  }'
```
...
- Public careers pages only - no authenticated ApplicantPro APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-140 for the design decision.
