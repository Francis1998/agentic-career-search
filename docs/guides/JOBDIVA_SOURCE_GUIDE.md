# JobDiva Source Guide

![JobDiva discovery flow](../../assets/demo/jobdiva-source.gif)

Use this guide when wiring a public JobDiva careers board into
...
JobDiva (`*.jobdiva.com`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/jd/{id}` detail paths. This adapter mirrors the Tribepad URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-jobdiva",
    "source_type": "jobdiva",
    "base_url": "https://acme.jobdiva.com/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/jd`),
...
- Public careers pages only - no authenticated JobDiva APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-133 for the design decision.
