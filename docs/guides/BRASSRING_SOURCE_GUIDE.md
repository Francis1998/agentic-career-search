# BrassRing Source Guide

![BrassRing discovery flow](../../assets/demo/brassring-source.gif)

Use this guide when wiring a public IBM Kenexa BrassRing careers board into
...
BrassRing (`*.brassring.com`) is an ATS / recruiting platform used by employers
that publish public careers pages. Boards commonly expose each job as an anchor
under `/jobs/{id}`, `/job/{id}`, `/jobdetail/{id}`, `/FgJobDetail/{id}`, or
`/careers/{id}` detail paths. This adapter mirrors the Beamery URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    ...
    "base_url": "https://acme.brassring.com/jobs/"
  }'
```
...
| Jobs | `/jobs/48291` |
| Job | `/job/55902` |
| Job detail | `/jobdetail/61033` |
| FgJobDetail | `/FgJobDetail/72044` |
| Careers | `/careers/83055` |

Board index pages (`/jobs`, `/careers`, `/jobdetail`),
...
- Public careers pages only - no authenticated BrassRing APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-139 for the design decision.
