...
![Dover discovery flow](../../assets/demo/dover-source.gif)

Use this guide when wiring a public Dover careers board into
...
Dover (`app.dover.com`) is an ATS popular with startups and growth-stage
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-dover",
    "source_type": "dover",
    "base_url": "https://app.dover.com/jobs/acme"
  }'
```
...
- Public careers pages only — no authenticated Dover APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-121 for the design decision.
