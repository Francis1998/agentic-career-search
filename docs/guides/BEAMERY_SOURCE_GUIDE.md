# Beamery Source Guide

![Beamery discovery flow](../../assets/demo/beamery-source.gif)

Use this guide when wiring a public Beamery careers board into
...
Beamery (`*.beamery.com`) is an ATS / recruiting platform used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/campaign/{id}` detail paths. This adapter mirrors the Tribepad URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-beamery",
    "source_type": "beamery",
    "base_url": "https://acme.beamery.com/jobs/"
  }'
```
...
| Campaign | `/campaign/83055` |

Board index pages (`/jobs`, `/careers`, `/campaign`),
...
- Public careers pages only - no authenticated Beamery APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-136 for the design decision.
