# CareerPlug Source Guide

![CareerPlug discovery flow](../../assets/demo/careerplug-source.gif)

Use this guide when wiring a public CareerPlug careers board into
...
CareerPlug (`{company}.careerplug.com`) is an ATS used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/listings/{id}` detail paths. This adapter mirrors the Jobylon URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-careerplug",
    "source_type": "careerplug",
    "base_url": "https://acme.careerplug.com/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/listings`),
...
- Public careers pages only - no authenticated CareerPlug APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-128 for the design decision.
