...
![HireEZ discovery flow](../../assets/demo/hireez-source.gif)

Use this guide when wiring a public HireEZ careers board into
...
HireEZ (`*.hireez.com`, formerly Hiretual) is an ATS / recruiting platform used
by employers that publish public careers pages. Boards commonly expose each job
as an anchor under `/jobs/{id}`, `/job/{id}`, `/careers/{id}`,
`/careers/job/{id}`, or `/positions/{id}` detail paths. This adapter mirrors the
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-hireez",
    "source_type": "hireez",
    "base_url": "https://acme.hireez.com/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/positions`),
...
- Public careers pages only - no authenticated HireEZ APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-138 for the design decision.
