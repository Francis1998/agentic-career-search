# TrackerRMS Source Guide

![TrackerRMS discovery flow](../../assets/demo/trackerrms-source.gif)

Use this guide when wiring a public TrackerRMS careers board into
...
TrackerRMS (`evoportal{region}.tracker-rms.com`) is an ATS used by employers
that publish public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/vacancies/{id}` detail paths. This adapter mirrors the Jobylon URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-trackerrms",
    "source_type": "trackerrms",
    "base_url": "https://evoportalus.tracker-rms.com/acme/jobs"
  }'
```
...
| Vacancies | `/vacancies/83055` |

Board index pages (`/jobs`, `/careers`, `/vacancies`),
...
- Public careers pages only - no authenticated TrackerRMS APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-127 for the design decision.
