# PCRecruiter Source Guide

![PCRecruiter discovery flow](../../assets/demo/pcrecruiter-source.gif)

Use this guide when wiring a public PCRecruiter careers board into
...
PCRecruiter (`*.pcrecruiter.com`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/requisition/{id}` detail paths. This adapter mirrors the Tribepad URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-pcrecruiter",
    "source_type": "pcrecruiter",
    "base_url": "https://acme.pcrecruiter.com/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/requisition`),
...
- Public careers pages only - no authenticated PCRecruiter APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-134 for the design decision.
