# Tribepad Source Guide

![Tribepad discovery flow](../../assets/demo/tribepad-source.gif)

Use this guide when wiring a public Tribepad careers board into
...
Tribepad (`*.tribepad.com`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/vacancy/{id}` detail paths. This adapter mirrors the CareerPlug URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-tribepad",
    "source_type": "tribepad",
    "base_url": "https://acme.tribepad.com/jobs/"
  }'
```
...
| Vacancy | `/vacancy/83055` |

Board index pages (`/jobs`, `/careers`, `/vacancy`),
...
- Public careers pages only - no authenticated Tribepad APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-131 for the design decision.
