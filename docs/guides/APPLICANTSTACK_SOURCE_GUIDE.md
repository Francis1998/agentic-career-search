# ApplicantStack Source Guide

![ApplicantStack discovery flow](../../assets/demo/applicantstack-source.gif)

Use this guide when wiring a public ApplicantStack careers board into
...
ApplicantStack (`{tenant}.applicantstack.com`) is an ATS used by employers that
publish public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/postings/{id}` detail paths. This adapter mirrors the Jobylon URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-applicantstack",
    "source_type": "applicantstack",
    "base_url": "https://acme.applicantstack.com/x/search"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/postings`),
...
- Public careers pages only - no authenticated ApplicantStack APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-126 for the design decision.
