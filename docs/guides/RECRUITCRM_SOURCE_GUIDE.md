# RecruitCRM Source Guide

![RecruitCRM discovery flow](../../assets/demo/recruitcrm-source.gif)

Use this guide when wiring a public RecruitCRM careers board into
...
RecruitCRM (`*.recruitcrm.io`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/opening/{id}` detail paths. This adapter mirrors the CareerPlug URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-recruitcrm",
    "source_type": "recruitcrm",
    "base_url": "https://acme.recruitcrm.io/jobs/"
  }'
```
...
- Public careers pages only - no authenticated RecruitCRM APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-129 for the design decision.
