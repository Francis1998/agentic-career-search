...
![Eploy discovery flow](../../assets/demo/eploy-source.gif)

Use this guide when wiring a public Eploy careers board into
...
eploy (`*.eploy.net`) is an ATS / recruiting platform used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/role/{id}` detail paths. This adapter mirrors the Tribepad URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-eploy",
    "source_type": "eploy",
    "base_url": "https://acme.eploy.net/jobs/"
  }'
```
...
Board index pages (`/jobs`, `/careers`, `/role`),
...
- Public careers pages only - no authenticated Eploy APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-137 for the design decision.
