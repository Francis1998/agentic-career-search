# Vincere Source Guide

![Vincere discovery flow](../../assets/demo/vincere-source.gif)

Use this guide when wiring a public Vincere careers board into
...
Vincere (`*.vincere.io`) is an ATS / recruiting CRM used by employers that publish
public careers pages. Boards commonly expose each job as an anchor under `/jobs/{id}`,
`/job/{id}`, `/careers/{id}`, `/careers/job/{id}`, or
`/job-detail/{id}` detail paths. This adapter mirrors the CareerPlug URL-shape
...
curl -X POST localhost:8000/source-configs \
  ...
  -d '{
    "name": "acme-vincere",
    "source_type": "vincere",
    "base_url": "https://acme.vincere.io/jobs/"
  }'
```
...
| Job detail | `/job-detail/83055` |

Board index pages (`/jobs`, `/careers`, `/job-detail`),
...
- Public careers pages only - no authenticated Vincere APIs.
- Outbound User-Agent comes from settings.
- Parsing stops at `max_jobs`; no unbounded crawl.

See ADR-130 for the design decision.
