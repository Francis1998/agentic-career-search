"""Unit tests for the TalentLyft careers site adapter."""
...
def test_talentlyft_parser_extracts_jobs() -> None:
    """TalentLyft parser should extract posting fields and skip nav/apply links."""

    adapter = TalentLyftAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://apply.talentlyft.com/acme/jobs/",
        TALENTLYFT_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "talentlyft"}
...
def test_talentlyft_parser_ignores_non_posting_links() -> None:
    """Only TalentLyft detail links should become candidates."""
...
def test_talentlyft_parser_falls_back_to_title_attribute() -> None:
...
def test_talentlyft_parser_honors_zero_max_jobs() -> None:
...
def test_talentlyft_location_is_scoped_to_its_posting() -> None:
