"""Unit tests for the Ceipal careers site adapter."""
...
<html>
  <body>
    ...
    <a href="/requisitions">Requisitions index</a>
...
def test_ceipal_parser_extracts_jobs() -> None:
    """Ceipal parser should extract posting fields and skip nav/apply links."""

    adapter = CeipalAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://jobs.ceipal.com/acme/jobs/",
        CEIPAL_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "ceipal"}
...
def test_ceipal_parser_ignores_non_posting_links() -> None:
    """Only Ceipal detail links should become candidates."""
...
def test_ceipal_parser_falls_back_to_title_attribute() -> None:
...
def test_ceipal_parser_honors_zero_max_jobs() -> None:
...
def test_ceipal_location_is_scoped_to_its_posting() -> None:
    ...
    assert by_title["Second Role"].location == "Hyderabad"
