"""Unit tests for the TrackerRMS careers site adapter."""
...
_PATH_E = f"/vacancies/{_JOB_E}"

TRACKERRMS_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_trackerrms_parser_extracts_jobs() -> None:
    """TrackerRMS parser should extract posting fields and skip nav/apply links."""

    adapter = TrackerRmsAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://evoportalus.tracker-rms.com/acme/jobs",
        TRACKERRMS_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "trackerrms"}
...
def test_trackerrms_parser_ignores_non_posting_links() -> None:
    """Only TrackerRMS detail links should become candidates."""
...
def test_trackerrms_parser_falls_back_to_title_attribute() -> None:
...
def test_trackerrms_parser_honors_zero_max_jobs() -> None:
...
def test_trackerrms_location_is_scoped_to_its_posting() -> None:
