"""Unit tests for the Join careers site adapter."""
...
_JOB_C = "61033"
_JOB_D = "72044"
_PATH_A = f"/companies/acme-corp/jobs/{_JOB_A}"
_PATH_B = f"/jobs/{_JOB_B}"
_PATH_C = f"/job/{_JOB_C}"
...
<html>
  <body>
    ...
    <a href="/companies/acme-corp/jobs">Company jobs index</a>
...
def test_join_parser_extracts_jobs() -> None:
    """Join parser should extract posting fields and skip nav/apply links."""

    adapter = JoinAdapter(user_agent="test-agent")
...
def test_join_parser_ignores_non_posting_links() -> None:
    """Only Join detail links should become candidates."""
...
def test_join_parser_falls_back_to_title_attribute() -> None:
...
def test_join_parser_honors_zero_max_jobs() -> None:
...
def test_join_location_is_scoped_to_its_posting() -> None:
