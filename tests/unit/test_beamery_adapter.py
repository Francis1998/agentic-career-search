"""Unit tests for the Beamery careers site adapter."""
...
_PATH_E = f"/campaign/{_JOB_E}"

BEAMERY_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_beamery_parser_extracts_jobs() -> None:
    """Beamery parser should extract posting fields and skip nav/apply links."""

    adapter = BeameryAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.beamery.com/jobs/",
        BEAMERY_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "beamery"}
...
def test_beamery_parser_ignores_non_posting_links() -> None:
    """Only Beamery detail links should become candidates."""
...
def test_beamery_parser_falls_back_to_title_attribute() -> None:
...
def test_beamery_parser_respects_max_jobs() -> None:
...
def test_beamery_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_beamery_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/campaign/22222"><h4>Second Role</h4></a>
