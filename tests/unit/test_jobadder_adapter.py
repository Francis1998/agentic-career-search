"""Unit tests for the JobAdder careers site adapter."""
  ...
  <body>
    <section class="openings">
      ...
      <article class="job">
        <a href="{_PATH_C}"
           class="heading"
           data-job-location="Melbourne • Remote">
...
def test_jobadder_parser_extracts_jobs() -> None:
    """JobAdder parser should extract posting fields and skip nav/apply links."""

    adapter = JobAdderAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.jobadder.com",
        JOBADDER_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Platform Engineer"].location == "Sydney"
    assert by_title["Implementation Engineer"].external_id == _JOB_C
    assert by_title["Implementation Engineer"].location == "Melbourne • Remote"
    assert by_title["Data Engineer"].external_id == _JOB_D
    assert by_title["Product Manager"].external_id == _JOB_E


def test_jobadder_parser_ignores_non_posting_links() -> None:
    """Only JobAdder detail links should become candidates."""

    adapter = JobAdderAdapter(user_agent="test-agent")
    nav_only_html = f"""
    <a href="/jobs">All jobs</a>
    <a href="/careers">Careers index</a>
    <a href="/position">Position index</a>
    <a href="{_PATH_A}/apply">Apply</a>
...
def test_jobadder_parser_falls_back_to_title_attribute() -> None:
...
def test_jobadder_parser_honors_zero_max_jobs() -> None:
...
def test_jobadder_location_is_scoped_to_its_posting() -> None:
    ...
    assert by_title["Second Role"].location == "Sydney"
