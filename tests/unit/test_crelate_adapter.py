"""Unit tests for the Crelate careers site adapter."""
...
_PATH_E = f"/opportunity/{_JOB_E}"

CRELATE_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_crelate_parser_extracts_jobs() -> None:
    """Crelate parser should extract posting fields and skip nav/apply links."""

    adapter = CrelateAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.crelate.com/jobs/",
        CRELATE_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "crelate"}
...
def test_crelate_parser_ignores_non_posting_links() -> None:
    """Only Crelate detail links should become candidates."""
...
def test_crelate_parser_falls_back_to_title_attribute() -> None:
...
def test_crelate_parser_respects_max_jobs() -> None:
...
def test_crelate_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_crelate_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/opportunity/22222"><h4>Second Role</h4></a>
