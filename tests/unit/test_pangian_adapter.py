"""Unit tests for the Pangian careers site adapter."""
...
def test_pangian_parser_extracts_jobs() -> None:
    """Pangian parser should extract posting fields and skip nav/apply links."""

    adapter = PangianAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://pangian.com/jobs",
        PANGIAN_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "pangian"}
...
def test_pangian_parser_ignores_non_posting_links() -> None:
    """Only Pangian detail links should become candidates."""
...
def test_pangian_parser_falls_back_to_title_attribute() -> None:
...
def test_pangian_parser_respects_max_jobs() -> None:
...
def test_pangian_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = PangianAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://pangian.com/jobs", PANGIAN_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://pangian.com/jobs", PANGIAN_SAMPLE_HTML, max_jobs=-1) == []


def test_pangian_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://pangian.com/jobs", html, max_jobs=10)
    assert len(jobs) == 1
