"""Unit tests for the SimplyHired careers site adapter."""
...
_B_PATH = "/jobs/56213"
_C = "62324"
_C_PATH = "/listings/62324"
_D = "73435"
_D_PATH = "/positions/73435"
_E = "84546"
_E_PATH = "/openings/84546"

SIMPLYHIRED_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/listings/62324" class="heading" data-job-location="Remote">
...
def test_simplyhired_parser_extracts_jobs() -> None:
    """SimplyHired parser should extract posting fields and skip nav/apply links."""

    adapter = SimplyhiredAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "simplyhired"}
...
def test_simplyhired_parser_ignores_non_posting_links() -> None:
    """Only SimplyHired detail links should become candidates."""
    ...
    <a href="/listings/{_C}/apply">Apply role</a>
    <a href="/about">About</a>
...
def test_simplyhired_parser_falls_back_to_title_attribute() -> None:
...
def test_simplyhired_parser_respects_max_jobs() -> None:
...
def test_simplyhired_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = SimplyhiredAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html(
            "https://www.simplyhired.com/search", SIMPLYHIRED_SAMPLE_HTML, max_jobs=0
        )
        == []
    )
    assert (
        adapter._parse_html(
            "https://www.simplyhired.com/search", SIMPLYHIRED_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_simplyhired_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://www.simplyhired.com/search", html, max_jobs=10)
    assert len(jobs) == 1
