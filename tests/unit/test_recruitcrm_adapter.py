"""Unit tests for the RecruitCRM careers site adapter."""
...
def test_recruitcrm_parser_extracts_jobs() -> None:
    """RecruitCRM parser should extract posting fields and skip nav/apply links."""

    adapter = RecruitCrmAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.recruitcrm.io/jobs/",
        RECRUITCRM_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "recruitcrm"}
...
def test_recruitcrm_parser_ignores_non_posting_links() -> None:
    """Only RecruitCRM detail links should become candidates."""
...
def test_recruitcrm_parser_falls_back_to_title_attribute() -> None:
...
def test_recruitcrm_parser_respects_max_jobs() -> None:
...
def test_recruitcrm_parser_returns_empty_for_non_positive_max_jobs() -> None:
