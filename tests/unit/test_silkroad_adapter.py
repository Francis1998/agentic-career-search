"""Unit tests for the SilkRoad Recruiting careers site adapter."""
...
_PATH_A = "/jobs/48291"
_PATH_B = "/job/55902"
_PATH_C = "/posting/61033"
_PATH_D = "/postings/72044"
_PATH_E = "/opportunity/83055"

SILKROAD_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/job/55902" class="heading" data-location="San Francisco">
          Platform Engineer
        </a>
      </article>
      <article class="job">
        <a href="/posting/61033" class="heading" data-job-location="New York">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/postings/72044" class="heading" data-location="Austin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/opportunity/83055" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
    ...
    <a href="/posting">Index</a>
    <a href="/opportunity">Index</a>
...
def test_silkroad_parser_extracts_jobs() -> None:
    """SilkRoad parser should extract posting fields and skip nav/apply links."""

    adapter = SilkRoadAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.silkroad.com/jobs/",
        SILKROAD_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "silkroad"}
...
def test_silkroad_parser_ignores_non_posting_links() -> None:
    """Only SilkRoad Recruiting detail links should become candidates."""
...
def test_silkroad_parser_falls_back_to_title_attribute() -> None:
...
def test_silkroad_parser_respects_max_jobs() -> None:
...
def test_silkroad_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = SilkRoadAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://acme.silkroad.com/jobs/", SILKROAD_SAMPLE_HTML, max_jobs=0)
        == []
    )
    assert (
        adapter._parse_html("https://acme.silkroad.com/jobs/", SILKROAD_SAMPLE_HTML, max_jobs=-1)
        == []
    )


def test_silkroad_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://acme.silkroad.com/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
