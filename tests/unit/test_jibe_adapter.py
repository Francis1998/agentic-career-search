"""Unit tests for the Jibe careers site adapter."""
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/requisition/83055" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
...
def test_jibe_parser_extracts_jobs() -> None:
    """Jibe parser should extract posting fields and skip nav/apply links."""

    adapter = JibeAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.jibe.com/jobs/",
        JIBE_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "jibe"}
...
def test_jibe_parser_ignores_non_posting_links() -> None:
    """Only Jibe detail links should become candidates."""
...
def test_jibe_parser_falls_back_to_title_attribute() -> None:
...
def test_jibe_parser_respects_max_jobs() -> None:
...
def test_jibe_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = JibeAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://acme.jibe.com/jobs/", JIBE_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://acme.jibe.com/jobs/", JIBE_SAMPLE_HTML, max_jobs=-1) == []


def test_jibe_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://acme.jibe.com/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
