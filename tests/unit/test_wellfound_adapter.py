"""Unit tests for the Wellfound careers site adapter."""
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/role/61033" class="heading" data-job-location="New York">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/roles/72044" class="heading" data-location="Austin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/startup-jobs/83055" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
...
def test_wellfound_parser_extracts_jobs() -> None:
    """Wellfound parser should extract posting fields and skip nav/apply links."""

    adapter = WellfoundAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "wellfound"}
...
def test_wellfound_parser_ignores_non_posting_links() -> None:
    """Only Wellfound detail links should become candidates."""
...
def test_wellfound_parser_falls_back_to_title_attribute() -> None:
...
def test_wellfound_parser_respects_max_jobs() -> None:
...
def test_wellfound_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = WellfoundAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html(
            "https://wellfound.com/company/acme/jobs/", WELLFOUND_SAMPLE_HTML, max_jobs=0
        )
        == []
    )
    assert (
        adapter._parse_html(
            "https://wellfound.com/company/acme/jobs/", WELLFOUND_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_wellfound_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://wellfound.com/company/acme/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
