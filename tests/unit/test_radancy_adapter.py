"""Unit tests for the Radancy careers site adapter."""
...
_PATH_A = "/jobs/48291"
_PATH_B = "/job/55902"
_PATH_C = "/search/job/61033"
_PATH_D = "/careers/72044"
_PATH_E = "/careers/job/83055"
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/search/job/61033" class="heading" data-job-location="New York">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/careers/72044" class="heading" data-location="Austin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/careers/job/83055" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
...
def test_radancy_parser_extracts_jobs() -> None:
    """Radancy parser should extract posting fields and skip nav/apply links."""

    adapter = RadancyAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.radancy.com/jobs/",
        RADANCY_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "radancy"}
...
def test_radancy_parser_ignores_non_posting_links() -> None:
    """Only Radancy detail links should become candidates."""
...
def test_radancy_parser_falls_back_to_title_attribute() -> None:
...
def test_radancy_parser_respects_max_jobs() -> None:
...
def test_radancy_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = RadancyAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://acme.radancy.com/jobs/", RADANCY_SAMPLE_HTML, max_jobs=0) == []
    )
    assert (
        adapter._parse_html("https://acme.radancy.com/jobs/", RADANCY_SAMPLE_HTML, max_jobs=-1)
        == []
    )


def test_radancy_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://acme.radancy.com/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
