"""Unit tests for the Paradox careers site adapter."""
...
def test_paradox_parser_extracts_jobs() -> None:
    """Paradox parser should extract posting fields and skip nav/apply links."""

    adapter = ParadoxAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.paradox.ai/jobs/",
        PARADOX_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "paradox"}
...
def test_paradox_parser_ignores_non_posting_links() -> None:
    """Only Paradox detail links should become candidates."""
...
def test_paradox_parser_falls_back_to_title_attribute() -> None:
...
def test_paradox_parser_respects_max_jobs() -> None:
...
def test_paradox_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_paradox_location_is_scoped_to_its_posting() -> None:
