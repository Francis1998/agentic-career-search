"""Unit tests for the HireVue careers site adapter."""
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/careers/61033" class="heading" data-job-location="New York">
...
def test_hirevue_parser_extracts_jobs() -> None:
    """HireVue parser should extract posting fields and skip nav/apply links."""

    adapter = HireVueAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.hirevue.com/jobs/",
        HIREVUE_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "hirevue"}
...
def test_hirevue_parser_ignores_non_posting_links() -> None:
    """Only HireVue detail links should become candidates."""
...
def test_hirevue_parser_falls_back_to_title_attribute() -> None:
...
def test_hirevue_parser_respects_max_jobs() -> None:
...
def test_hirevue_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = HireVueAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://acme.hirevue.com/jobs/", HIREVUE_SAMPLE_HTML, max_jobs=0) == []
    )
    assert (
        adapter._parse_html("https://acme.hirevue.com/jobs/", HIREVUE_SAMPLE_HTML, max_jobs=-1)
        == []
    )


def test_hirevue_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://acme.hirevue.com/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
