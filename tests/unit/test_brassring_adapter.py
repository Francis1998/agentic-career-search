"""Unit tests for the BrassRing careers site adapter."""
...
_PATH_C = f"/jobdetail/{_JOB_C}"
_PATH_D = f"/FgJobDetail/{_JOB_D}"
_PATH_E = f"/careers/{_JOB_E}"

BRASSRING_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_brassring_parser_extracts_jobs() -> None:
    """BrassRing parser should extract posting fields and skip nav/apply links."""

    adapter = BrassringAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.brassring.com/jobs/",
        BRASSRING_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "brassring"}
...
def test_brassring_parser_ignores_non_posting_links() -> None:
    """Only BrassRing detail links should become candidates."""
...
def test_brassring_parser_falls_back_to_title_attribute() -> None:
...
def test_brassring_parser_respects_max_jobs() -> None:
...
def test_brassring_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_brassring_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/jobdetail/22222"><h4>Second Role</h4></a>
