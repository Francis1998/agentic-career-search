"""Unit tests for the ZipRecruiter careers site adapter."""
...
def test_ziprecruiter_parser_extracts_jobs() -> None:
    """ZipRecruiter parser should extract posting fields and skip nav/apply links."""

    adapter = ZiprecruiterAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "ziprecruiter"}
...
def test_ziprecruiter_parser_ignores_non_posting_links() -> None:
    """Only ZipRecruiter detail links should become candidates."""
...
def test_ziprecruiter_parser_falls_back_to_title_attribute() -> None:
...
def test_ziprecruiter_parser_respects_max_jobs() -> None:
...
def test_ziprecruiter_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = ZiprecruiterAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html(
            "https://www.ziprecruiter.com/jobs", ZIPRECRUITER_SAMPLE_HTML, max_jobs=0
        )
        == []
    )
    assert (
        adapter._parse_html(
            "https://www.ziprecruiter.com/jobs", ZIPRECRUITER_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_ziprecruiter_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://www.ziprecruiter.com/jobs", html, max_jobs=10)
    assert len(jobs) == 1
