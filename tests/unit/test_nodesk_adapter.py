"""Unit tests for the NoDesk careers site adapter."""
...
_PATH_B = "/job/55902"
_JOB_C = "61033"
_PATH_C = "/remote/61033"
_JOB_D = "72044"
_PATH_D = "/positions/72044"
_JOB_E = "83055"
_PATH_E = "/careers/83055"
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/remote/61033" class="heading" data-job-location="Remote">
      ...
      <article class="job">
        <a href="/careers/83055" class="heading" data-location="Tokyo">
          Product Manager
        </a>
      </article>
    </section>
...
def test_nodesk_parser_extracts_jobs() -> None:
    """NoDesk parser should extract posting fields and skip nav/apply links."""

    adapter = NodeskAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://nodesk.co/jobs",
        NODESK_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "nodesk"}
...
def test_nodesk_parser_ignores_non_posting_links() -> None:
    """Only NoDesk detail links should become candidates."""
    ...
    <a href="/remote/{_JOB_C}/apply">Apply remote</a>
    <a href="/about">About</a>
...
def test_nodesk_parser_falls_back_to_title_attribute() -> None:
...
def test_nodesk_parser_respects_max_jobs() -> None:
...
def test_nodesk_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = NodeskAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://nodesk.co/jobs", NODESK_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://nodesk.co/jobs", NODESK_SAMPLE_HTML, max_jobs=-1) == []


def test_nodesk_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://nodesk.co/jobs", html, max_jobs=10)
    assert len(jobs) == 1
