"""Unit tests for the JustRemote careers site adapter."""
...
JUSTREMOTE_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      <article class="job">
        <a href="/remote-jobs/48291" class="heading" data-location="Lisbon">
          Software Engineer
        </a>
      </article>
      <article class="job">
        <a href="/jobs/55902" class="heading" data-location="Bali">
          Platform Engineer
        </a>
      </article>
      <article class="job">
        <a href="/job/61033" class="heading" data-job-location="Remote">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/listings/72044" class="heading" data-location="Berlin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/listing/83055" class="heading" data-location="Tokyo">
          Product Manager
        </a>
      </article>
    </section>
...
def test_justremote_parser_extracts_jobs() -> None:
    """JustRemote parser should extract posting fields and skip nav/apply links."""

    adapter = JustremoteAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "justremote"}
...
def test_justremote_parser_ignores_non_posting_links() -> None:
    """Only JustRemote detail links should become candidates."""
    ...
    <a href="/remote-jobs/{_JOB_A}/login">Login</a>
    <a href="/job/{_JOB_C}/apply">Apply remote</a>
    <a href="/about">About</a>
...
def test_justremote_parser_falls_back_to_title_attribute() -> None:
...
def test_justremote_parser_respects_max_jobs() -> None:
...
def test_justremote_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = JustremoteAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://justremote.co/remote-jobs", JUSTREMOTE_SAMPLE_HTML, max_jobs=0)
        == []
    )
    assert (
        adapter._parse_html(
            "https://justremote.co/remote-jobs", JUSTREMOTE_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_justremote_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://justremote.co/remote-jobs", html, max_jobs=10)
    assert len(jobs) == 1
