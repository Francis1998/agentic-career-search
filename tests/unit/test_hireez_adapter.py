"""Unit tests for the HireEZ careers site adapter."""
...
def test_hireez_parser_extracts_jobs() -> None:
    """HireEZ parser should extract posting fields and skip nav/apply links."""

    adapter = HireezAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.hireez.com/jobs/",
        HIREEZ_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "hireez"}
...
def test_hireez_parser_ignores_non_posting_links() -> None:
    """Only HireEZ detail links should become candidates."""
...
def test_hireez_parser_falls_back_to_title_attribute() -> None:
...
def test_hireez_parser_respects_max_jobs() -> None:
...
def test_hireez_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_hireez_location_is_scoped_to_its_posting() -> None:
