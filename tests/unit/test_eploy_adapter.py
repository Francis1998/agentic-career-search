"""Unit tests for the Eploy careers site adapter."""
...
def test_eploy_parser_extracts_jobs() -> None:
    """Eploy parser should extract posting fields and skip nav/apply links."""

    adapter = EployAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.eploy.net/jobs/",
        EPLOY_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "eploy"}
...
def test_eploy_parser_ignores_non_posting_links() -> None:
    """Only Eploy detail links should become candidates."""
...
def test_eploy_parser_falls_back_to_title_attribute() -> None:
...
def test_eploy_parser_respects_max_jobs() -> None:
...
def test_eploy_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_eploy_location_is_scoped_to_its_posting() -> None:
