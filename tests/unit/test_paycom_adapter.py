"""Unit tests for the Paycom careers site adapter."""
...
def test_paycom_parser_extracts_jobs() -> None:
    """Paycom parser should extract posting fields and skip nav/apply links."""

    adapter = PaycomAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.paycomonline.net/jobs/",
        PAYCOM_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "paycom"}
...
def test_paycom_parser_ignores_non_posting_links() -> None:
    """Only Paycom detail links should become candidates."""
...
def test_paycom_parser_falls_back_to_title_attribute() -> None:
...
def test_paycom_parser_respects_max_jobs() -> None:
...
def test_paycom_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = PaycomAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://acme.paycomonline.net/jobs/", PAYCOM_SAMPLE_HTML, max_jobs=0)
        == []
    )
    assert (
        adapter._parse_html("https://acme.paycomonline.net/jobs/", PAYCOM_SAMPLE_HTML, max_jobs=-1)
        == []
    )


def test_paycom_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://acme.paycomonline.net/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
