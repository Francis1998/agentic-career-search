"""Unit tests for the ClearCompany careers site adapter."""
...
_PATH_B = f"/careers/{_JOB_B}"
_PATH_C = f"/jobs/{_JOB_C}"
_PATH_D = f"/job/{_JOB_D}-software-engineer"
_PATH_E = f"/position/{_JOB_E}"

CLEARCOMPANY_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_clearcompany_parser_extracts_jobs() -> None:
    """ClearCompany parser should extract posting fields and skip nav/apply links."""

    adapter = ClearCompanyAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.clearcompany.com",
        CLEARCOMPANY_SAMPLE_HTML,
        max_jobs=10,
    )
...
def test_clearcompany_parser_ignores_non_posting_links() -> None:
    """Only ClearCompany detail links should become candidates."""
    ...
    <a href="{_PATH_A}/apply">Apply</a>
    <a href="/careers/job/{_JOB_A}/login">Login</a>
    <a href="/about">About</a>
...
def test_clearcompany_parser_falls_back_to_title_attribute() -> None:
...
def test_clearcompany_parser_honors_zero_max_jobs() -> None:
...
def test_clearcompany_location_is_scoped_to_its_posting() -> None:
