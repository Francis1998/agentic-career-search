"""Unit tests for the Jooble careers site adapter."""
...
_B_PATH = "/job/56213"
_C = "62324"
_C_PATH = "/vacancy/62324"
_D = "73435"
_D_PATH = "/vacancies/73435"
_E = "84546"
_E_PATH = "/listings/84546"
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/vacancy/62324" class="heading" data-job-location="Remote">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/vacancies/73435" class="heading" data-location="Berlin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/listings/84546" class="heading" data-location="Tokyo">
          Product Manager
        </a>
      </article>
    </section>
...
def test_jooble_parser_extracts_jobs() -> None:
    """Jooble parser should extract posting fields and skip nav/apply links."""

    adapter = JoobleAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "jooble"}
...
def test_jooble_parser_ignores_non_posting_links() -> None:
    """Only Jooble detail links should become candidates."""
    ...
    <a href="/vacancy/{_C}/apply">Apply role</a>
    <a href="/about">About</a>
...
def test_jooble_parser_falls_back_to_title_attribute() -> None:
...
def test_jooble_parser_respects_max_jobs() -> None:
...
def test_jooble_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = JoobleAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://jooble.org/jobs", JOOBLE_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://jooble.org/jobs", JOOBLE_SAMPLE_HTML, max_jobs=-1) == []


def test_jooble_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://jooble.org/jobs", html, max_jobs=10)
    assert len(jobs) == 1
