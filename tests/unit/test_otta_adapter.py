"""Unit tests for the Otta careers site adapter."""
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/roles/61033" class="heading" data-job-location="New York">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="/role/72044" class="heading" data-location="Austin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/openings/83055" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
...
def test_otta_parser_extracts_jobs() -> None:
    """Otta parser should extract posting fields and skip nav/apply links."""

    adapter = OttaAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "otta"}
...
def test_otta_parser_ignores_non_posting_links() -> None:
    """Only Otta detail links should become candidates."""
...
def test_otta_parser_falls_back_to_title_attribute() -> None:
...
def test_otta_parser_respects_max_jobs() -> None:
...
def test_otta_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = OttaAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://app.otta.com/jobs/", OTTA_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://app.otta.com/jobs/", OTTA_SAMPLE_HTML, max_jobs=-1) == []


def test_otta_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://app.otta.com/jobs/", html, max_jobs=10)
    assert len(jobs) == 1
