"""Unit tests for the ApplicantPro careers site adapter."""
...
def test_applicantpro_parser_extracts_jobs() -> None:
    """ApplicantPro parser should extract posting fields and skip nav/apply links."""

    adapter = ApplicantProAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.applicantpro.com/jobs/",
        APPLICANTPRO_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "applicantpro"}
...
def test_applicantpro_parser_ignores_non_posting_links() -> None:
    """Only ApplicantPro detail links should become candidates."""
...
def test_applicantpro_parser_falls_back_to_title_attribute() -> None:
...
def test_applicantpro_parser_respects_max_jobs() -> None:
...
def test_applicantpro_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_applicantpro_location_is_scoped_to_its_posting() -> None:
