"""Unit tests for the Polymer careers site adapter."""
...
def test_polymer_parser_extracts_jobs() -> None:
    """Polymer parser should extract posting fields and skip nav/apply links."""

    adapter = PolymerAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.polymer.co",
        POLYMER_SAMPLE_HTML,
        max_jobs=10,
    )
...
def test_polymer_parser_ignores_non_posting_links() -> None:
    """Only Polymer detail links should become candidates."""
...
def test_polymer_parser_falls_back_to_title_attribute() -> None:
...
def test_polymer_parser_honors_zero_max_jobs() -> None:
...
def test_polymer_location_is_scoped_to_its_posting() -> None:
