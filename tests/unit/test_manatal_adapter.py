"""Unit tests for the Manatal careers site adapter."""
...
def test_manatal_parser_extracts_jobs() -> None:
    """Manatal parser should extract posting fields and skip nav/apply links."""

    adapter = ManatalAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.manatal.com",
        MANATAL_SAMPLE_HTML,
        max_jobs=10,
    )
...
def test_manatal_parser_ignores_non_posting_links() -> None:
    """Only Manatal detail links should become candidates."""
...
def test_manatal_parser_falls_back_to_title_attribute() -> None:
...
def test_manatal_parser_honors_zero_max_jobs() -> None:
...
def test_manatal_location_is_scoped_to_its_posting() -> None:
