"""Unit tests for the Paylocity careers site adapter."""
  ...
  <body>
    <section class="openings">
      ...
      <article class="job">
        <a href="{_PATH_C}"
           class="heading"
           data-job-location="Austin • Remote">
...
def test_paylocity_parser_extracts_jobs() -> None:
    """Paylocity parser should extract posting fields and skip nav/apply links."""

    adapter = PaylocityAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://recruiting.paylocity.com/recruiting/jobs/All/acme",
        PAYLOCITY_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Platform Engineer"].location == "Chicago"
    assert by_title["Implementation Engineer"].external_id == _JOB_C
    assert by_title["Implementation Engineer"].location == "Austin • Remote"
    assert by_title["Data Engineer"].external_id == _JOB_D
    assert by_title["Product Manager"].external_id == _JOB_E


def test_paylocity_parser_ignores_non_posting_links() -> None:
    """Only Paylocity detail links should become candidates."""
    ...
    <a href="/JobDetails">Job details index</a>
    <a href="{_PATH_A}/apply">Apply</a>
...
def test_paylocity_parser_falls_back_to_title_attribute() -> None:
...
def test_paylocity_parser_honors_zero_max_jobs() -> None:
...
def test_paylocity_location_is_scoped_to_its_posting() -> None:
    ...
    assert by_title["Second Role"].location == "Chicago"
