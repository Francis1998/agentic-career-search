"""Unit tests for the Work at a Startup careers site adapter."""
...
_A = "50113"
_PATH_A = "/companies/50113"
_B = "57224"
_PATH_B = "/startups/57224"
_C = "63335"
_PATH_C = "/roles/63335"
_D = "74446"
_PATH_D = "/jobs/74446"
...
Workatastartup_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      <article class="job">
        <a href="/companies/50113" class="heading" data-location="SF">
          Software Engineer
        </a>
      </article>
      <article class="job">
        <a href="/startups/57224" class="heading" data-location="NYC">
          Platform Engineer
        </a>
      </article>
      <article class="job">
        <a href="/roles/63335" class="heading" data-job-location="Remote">
    ...
    <a href="/jobs">Index</a>
    <a href="/job">Index</a>
    <a href="/companies/50113/apply">Apply</a>
...
def test_workatastartup_parser_extracts_jobs() -> None:
    """Work at a Startup parser should extract posting fields and skip nav/apply links."""

    adapter = WorkatastartupAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].location == "SF"
    assert by_title["Software Engineer"].url.endswith(_PATH_A)
    assert by_title["Software Engineer"].raw == {"source": "workatastartup"}
    assert by_title["Platform Engineer"].external_id == _B
    assert by_title["Platform Engineer"].location == "NYC"
...
def test_workatastartup_parser_ignores_non_posting_links() -> None:
    """Only Work at a Startup detail links should become candidates."""
    ...
    <a href="/companies/{_A}/login">Login</a>
...
def test_workatastartup_parser_falls_back_to_title_attribute() -> None:
...
def test_workatastartup_parser_respects_max_jobs() -> None:
...
def test_workatastartup_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = WorkatastartupAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html(
            "https://www.workatastartup.com/jobs", Workatastartup_SAMPLE_HTML, max_jobs=0
        )
        == []
    )
    assert (
        adapter._parse_html(
            "https://www.workatastartup.com/jobs", Workatastartup_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_workatastartup_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://www.workatastartup.com/jobs", html, max_jobs=10)
    assert len(jobs) == 1
