"""Unit tests for the Bullhorn careers site adapter."""
...
def test_bullhorn_parser_extracts_jobs() -> None:
    """Bullhorn parser should extract posting fields and skip nav/apply links."""

    adapter = BullhornAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.bullhornstaffing.com",
        BULLHORN_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Platform Engineer"].location == "Boston"
...
def test_bullhorn_parser_ignores_non_posting_links() -> None:
    """Only Bullhorn detail links should become candidates."""
...
def test_bullhorn_parser_falls_back_to_title_attribute() -> None:
...
def test_bullhorn_parser_honors_zero_max_jobs() -> None:
...
def test_bullhorn_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      ...
      <span class="job-location">Boston</span>
    </div>
    """
    jobs = adapter._parse_html(
        "https://acme.bullhornstaffing.com",
    ...
    assert by_title["Second Role"].location == "Boston"
