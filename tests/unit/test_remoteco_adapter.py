"""Unit tests for the Remoteco careers site adapter."""
...
def test_remoteco_parser_extracts_jobs() -> None:
    """Remoteco parser should extract posting fields and skip nav/apply links."""

    adapter = RemotecoAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "remoteco"}
...
def test_remoteco_parser_ignores_non_posting_links() -> None:
    """Only Remoteco detail links should become candidates."""
...
def test_remoteco_parser_falls_back_to_title_attribute() -> None:
...
def test_remoteco_parser_respects_max_jobs() -> None:
...
def test_remoteco_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = RemotecoAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://remote.co/remote-jobs", REMOTECO_SAMPLE_HTML, max_jobs=0) == []
    )
    assert (
        adapter._parse_html("https://remote.co/remote-jobs", REMOTECO_SAMPLE_HTML, max_jobs=-1)
        == []
    )


def test_remoteco_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://remote.co/remote-jobs", html, max_jobs=10)
    assert len(jobs) == 1
