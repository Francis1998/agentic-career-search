"""Unit tests for the Recruiterflow careers site adapter."""
...
_PATH_C = f"/careers/job/{_JOB_C}"
_PATH_D = f"/openings/{_JOB_D}"
_PATH_E = f"/opening/{_JOB_E}"

RECRUITERFLOW_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_recruiterflow_parser_extracts_jobs() -> None:
    """Recruiterflow parser should extract posting fields and skip nav/apply links."""

    adapter = RecruiterflowAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.recruiterflow.com",
        RECRUITERFLOW_SAMPLE_HTML,
        max_jobs=10,
    )
...
def test_recruiterflow_parser_ignores_non_posting_links() -> None:
    """Only Recruiterflow detail links should become candidates."""

    adapter = RecruiterflowAdapter(user_agent="test-agent")
    nav_only_html = f"""
    <a href="/jobs">All jobs</a>
    <a href="/openings">Openings index</a>
    <a href="/opening">Opening index</a>
    <a href="{_PATH_A}/apply">Apply</a>
...
def test_recruiterflow_parser_falls_back_to_title_attribute() -> None:
...
def test_recruiterflow_parser_honors_zero_max_jobs() -> None:
...
def test_recruiterflow_location_is_scoped_to_its_posting() -> None:
