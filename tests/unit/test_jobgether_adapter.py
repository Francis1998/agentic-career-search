"""Unit tests for the Jobgether careers site adapter."""
...
JOBGETHER_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      <article class="job">
        <a href="/roles/48291" class="heading" data-location="Lisbon">
          Software Engineer
        </a>
      </article>
      <article class="job">
        <a href="/openings/55902" class="heading" data-location="Bali">
          Platform Engineer
        </a>
      </article>
      <article class="job">
    ...
    <a href="/jobs">Index</a>
    <a href="/job">Index</a>
    <a href="/roles/48291/apply">Apply</a>
...
def test_jobgether_parser_extracts_jobs() -> None:
    """Jobgether parser should extract posting fields and skip nav/apply links."""

    adapter = JobgetherAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://jobgether.com/jobs",
        JOBGETHER_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "jobgether"}
...
def test_jobgether_parser_ignores_non_posting_links() -> None:
    """Only Jobgether detail links should become candidates."""
    ...
    <a href="/roles/{_JOB_A}/login">Login</a>
...
def test_jobgether_parser_falls_back_to_title_attribute() -> None:
...
def test_jobgether_parser_respects_max_jobs() -> None:
...
def test_jobgether_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = JobgetherAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html("https://jobgether.com/jobs", JOBGETHER_SAMPLE_HTML, max_jobs=0) == []
    )
    assert (
        adapter._parse_html("https://jobgether.com/jobs", JOBGETHER_SAMPLE_HTML, max_jobs=-1) == []
    )


def test_jobgether_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://jobgether.com/jobs", html, max_jobs=10)
    assert len(jobs) == 1
