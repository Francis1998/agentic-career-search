"""Unit tests for the Working Nomads careers site adapter."""
...
_PATH_E = "/position/83055"

WORKINGNOMADS_SAMPLE_HTML = """
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/positions/72044" class="heading" data-location="Chiang Mai">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="/position/83055" class="heading" data-location="Berlin">
          Product Manager
        </a>
      </article>
    </section>
...
def test_workingnomads_parser_extracts_jobs() -> None:
    """Working Nomads parser should extract posting fields and skip nav/apply links."""

    adapter = WorkingnomadsAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "workingnomads"}
...
def test_workingnomads_parser_ignores_non_posting_links() -> None:
    """Only Working Nomads detail links should become candidates."""
...
def test_workingnomads_parser_falls_back_to_title_attribute() -> None:
...
def test_workingnomads_parser_respects_max_jobs() -> None:
...
def test_workingnomads_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = WorkingnomadsAdapter(user_agent="test-agent")
    assert (
        adapter._parse_html(
            "https://www.workingnomads.com/jobs", WORKINGNOMADS_SAMPLE_HTML, max_jobs=0
        )
        == []
    )
    assert (
        adapter._parse_html(
            "https://www.workingnomads.com/jobs", WORKINGNOMADS_SAMPLE_HTML, max_jobs=-1
        )
        == []
    )


def test_workingnomads_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://www.workingnomads.com/jobs", html, max_jobs=10)
    assert len(jobs) == 1
