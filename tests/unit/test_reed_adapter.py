"""Unit tests for the Reed careers site adapter."""
...
<html>
  <body>
    <section class="listings">
      ...
      <article class="job">
        <a href="/details/62324" class="heading" data-job-location="Remote">
...
def test_reed_parser_extracts_jobs() -> None:
    """Reed parser should extract posting fields and skip nav/apply links."""

    adapter = ReedAdapter(user_agent="test-agent")
    ...
    assert by_title["Software Engineer"].raw == {"source": "reed"}
...
def test_reed_parser_ignores_non_posting_links() -> None:
    """Only Reed detail links should become candidates."""
    ...
    <a href="/details/{_C}/apply">Apply role</a>
    <a href="/about">About</a>
...
def test_reed_parser_falls_back_to_title_attribute() -> None:
...
def test_reed_parser_respects_max_jobs() -> None:
...
def test_reed_parser_returns_empty_for_non_positive_max_jobs() -> None:
    """Non-positive max_jobs should short-circuit to an empty list."""

    adapter = ReedAdapter(user_agent="test-agent")
    assert adapter._parse_html("https://www.reed.co.uk/jobs", REED_SAMPLE_HTML, max_jobs=0) == []
    assert adapter._parse_html("https://www.reed.co.uk/jobs", REED_SAMPLE_HTML, max_jobs=-1) == []


def test_reed_parser_uses_remote_flag() -> None:
    ...
    """
    jobs = adapter._parse_html("https://www.reed.co.uk/jobs", html, max_jobs=10)
    assert len(jobs) == 1
