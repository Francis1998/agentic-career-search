"""Unit tests for the Loxo careers site adapter."""
...
_PATH_C = f"/positions/{_JOB_C}"
_PATH_D = f"/careers/{_JOB_D}"
...
def test_loxo_parser_extracts_jobs() -> None:
    """Loxo parser should extract posting fields and skip nav/apply links."""

    adapter = LoxoAdapter(user_agent="test-agent")
    ...
    assert by_title["Platform Engineer"].location == "New York"
...
def test_loxo_parser_ignores_non_posting_links() -> None:
    """Only Loxo detail links should become candidates."""
...
def test_loxo_parser_falls_back_to_title_attribute() -> None:
...
def test_loxo_parser_honors_zero_max_jobs() -> None:
...
def test_loxo_location_is_scoped_to_its_posting() -> None:
    ...
    assert by_title["Second Role"].location == "New York"
