"""Unit tests for the CareerPlug careers site adapter."""
...
_PATH_E = f"/listings/{_JOB_E}"

CAREERPLUG_SAMPLE_HTML = f"""
<html>
  <body>
    ...
    <a href="/listings">Listings index</a>
...
def test_careerplug_parser_extracts_jobs() -> None:
    """CareerPlug parser should extract posting fields and skip nav/apply links."""

    adapter = CareerPlugAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.careerplug.com/jobs/",
        CAREERPLUG_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "careerplug"}
...
def test_careerplug_parser_ignores_non_posting_links() -> None:
    """Only CareerPlug detail links should become candidates."""
...
def test_careerplug_parser_falls_back_to_title_attribute() -> None:
...
def test_careerplug_parser_honors_zero_max_jobs() -> None:
...
def test_careerplug_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/listings/22222"><h4>Second Role</h4></a>
