"""Unit tests for the Cornerstone careers site adapter."""
...
_PATH_E = f"/opening/{_JOB_E}"

CORNERSTONE_SAMPLE_HTML = f"""
<html>
  <body>
...
def test_cornerstone_parser_extracts_jobs() -> None:
    """Cornerstone parser should extract posting fields and skip nav/apply links."""

    adapter = CornerstoneAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.csod.com/jobs/",
        CORNERSTONE_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "cornerstone"}
...
def test_cornerstone_parser_ignores_non_posting_links() -> None:
    """Only Cornerstone detail links should become candidates."""
...
def test_cornerstone_parser_falls_back_to_title_attribute() -> None:
...
def test_cornerstone_parser_respects_max_jobs() -> None:
...
def test_cornerstone_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_cornerstone_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/opening/22222"><h4>Second Role</h4></a>
