"""Unit tests for the CATS careers site adapter."""
...
_PATH_E = f"/postings/{_JOB_E}"

CATSONE_SAMPLE_HTML = f"""
<html>
  <body>
    <section class="listings">
      <article class="job">
        <a href="{_PATH_A}" class="heading" data-location="Remote">
          Software Engineer
        </a>
      </article>
      <article class="job">
        <a href="{_PATH_B}" class="heading" data-location="San Francisco">
          Platform Engineer
        </a>
      </article>
      <article class="job">
        <a href="{_PATH_C}" class="heading" data-job-location="New York">
          Implementation Engineer
        </a>
      </article>
      <article class="job">
        <a href="{_PATH_D}" class="heading" data-location="Austin">
          Data Engineer
        </a>
      </article>
      <article class="job">
        <a href="{_PATH_E}" class="heading" data-location="London">
          Product Manager
        </a>
      </article>
    </section>
...
def test_catsone_parser_extracts_jobs() -> None:
    """CATS parser should extract posting fields and skip nav/apply links."""

    adapter = CatsoneAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.catsone.com/jobs/",
        CATSONE_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Software Engineer"].raw == {"source": "catsone"}
...
def test_catsone_parser_ignores_non_posting_links() -> None:
    """Only CATS detail links should become candidates."""
...
def test_catsone_parser_falls_back_to_title_attribute() -> None:
...
def test_catsone_parser_respects_max_jobs() -> None:
...
def test_catsone_parser_returns_empty_for_non_positive_max_jobs() -> None:
...
def test_catsone_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/postings/22222"><h4>Second Role</h4></a>
