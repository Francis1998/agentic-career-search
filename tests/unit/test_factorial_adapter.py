"""Unit tests for the Factorial HR careers site adapter."""
...
_PATH_E = f"/open-positions/{_JOB_E}"

FACTORIAL_SAMPLE_HTML = f"""
<html>
  <body>
    <section class="openings">
      ...
      <article class="job">
        <a href="{_PATH_C}"
           class="heading"
           data-job-location="Madrid • Remote">
...
def test_factorial_parser_extracts_jobs() -> None:
    """Factorial parser should extract posting fields and skip nav/apply links."""

    adapter = FactorialAdapter(user_agent="test-agent")
    jobs = adapter._parse_html(
        "https://acme.factorialhr.com",
        FACTORIAL_SAMPLE_HTML,
        max_jobs=10,
    )
    ...
    assert by_title["Platform Engineer"].location == "Barcelona"
    assert by_title["Implementation Engineer"].external_id == _JOB_C
    assert by_title["Implementation Engineer"].location == "Madrid • Remote"
    assert by_title["Data Engineer"].external_id == _JOB_D
    assert by_title["Product Manager"].external_id == _JOB_E


def test_factorial_parser_ignores_non_posting_links() -> None:
    """Only Factorial detail links should become candidates."""

    adapter = FactorialAdapter(user_agent="test-agent")
    nav_only_html = f"""
    <a href="/jobs">All jobs</a>
    <a href="/careers">Careers index</a>
    <a href="/open-positions">Open positions index</a>
    <a href="{_PATH_A}/apply">Apply</a>
...
def test_factorial_parser_falls_back_to_title_attribute() -> None:
...
def test_factorial_parser_honors_zero_max_jobs() -> None:
...
def test_factorial_location_is_scoped_to_its_posting() -> None:
    ...
    <div class="job">
      <a href="/open-positions/22222"><h4>Second Role</h4></a>
      <span class="job-location">Barcelona</span>
    </div>
    """
    jobs = adapter._parse_html(
        "https://acme.factorialhr.com",
    ...
    assert by_title["Second Role"].location == "Barcelona"
