...
- `NonCompeteRestrictivenessFlagger` (`services/noncompete_flagger.py`): offline HITL non-compete duration/geography/garden-leave restrictiveness bands (never auto-accepts). Gap vs Blind/Levels.fyi/Candor offer reviews. Optional later polish via GPT-5.5 / Claude Sonnet 4.6 / Gemini 3.x / Kimi K2. See `docs/guides/NONCOMPETE_RESTRICTIVENESS_FLAGGER_GUIDE.md`.
- `EquityVestingCliffAdvisor` (`services/equity_vesting.py`): offline HITL cliff + linear vesting progress for a grant (never auto-accepts). Gap vs Levels.fyi/Candor/Carta vesting calculators. Optional later polish via GPT-5.5 / Claude Sonnet 4.6 / Gemini 3.x / Kimi K2. See `docs/guides/EQUITY_VESTING_CLIFF_ADVISOR_GUIDE.md`.
...
- `StarBehavioralStoryMatcher` (`services/star_story.py`): offline HITL STAR story↔JD competency matcher (never auto-sends). Gap vs Interviewing.io/Exponent/Teal behavioral banks. Optional later polish via GPT-5.5 / Claude Sonnet 4.6 / Gemini 3.x / Kimi K2. See `docs/guides/STAR_BEHAVIORAL_STORY_MATCHER_GUIDE.md`.
...
- `HimalayasAdapter` (`source_type: himalayas`): public Himalayas careers boards (himalayas.app). Postings are recognised by `/jobs/{id}`, `/job/{id}`, `/companies/{slug}/jobs/{id}`, `/remote-jobs/{id}`, `/roles/{id}` URL shapes; board indexes, bare company pages, apply/login/signin/about/index steps, and navigation links are ignored. See ADR-157 and `docs/guides/HIMALAYAS_SOURCE_GUIDE.md`.
