...
### Added
- `PriorityAlertWebhook` (`services/alerting.py`): fail-soft sync/async helper that POSTs JSON to a configured webhook when `priority_tier == "high"`. Config flags: `PRIORITY_ALERT_WEBHOOK_ENABLED`, `PRIORITY_ALERT_WEBHOOK_URL`, `PRIORITY_ALERT_WEBHOOK_TIMEOUT_SECONDS`. See `docs/guides/PRIORITY_ALERT_WEBHOOK_GUIDE.md`.
