"""HiBob public careers site adapter.

HiBob (Bob)-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/positions/{id}``. This adapter
...
class HibobAdapter(CareerSourceAdapter):
    """Fetch jobs from public HiBob (Bob) careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse HiBob jobs.

        Args:
            base_url: HiBob careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse HiBob careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a HiBob posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised HiBob job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a HiBob URL.

        Args:
            job_url: HiBob job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid HiBob posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a HiBob job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a HiBob anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from HiBob anchor metadata or markup."""
