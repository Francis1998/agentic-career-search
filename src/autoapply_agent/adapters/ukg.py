"""UKG public careers site adapter.

UKG-hosted public careers boards expose listing pages whose postings render as
anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/opportunities/{id}``, ``/opportunity/{id}``, and ``/careers/job/{id}``. This
...
class UkgAdapter(CareerSourceAdapter):
    """Fetch jobs from public UKG careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse UKG jobs.

        Args:
            base_url: UKG careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse UKG careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a UKG posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised UKG job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a UKG URL.

        Args:
            job_url: UKG job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid UKG posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a UKG job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a UKG anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from UKG anchor metadata or markup."""
