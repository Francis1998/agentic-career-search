"""Vincere public careers site adapter.

Vincere-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/job-detail/{id}``. This adapter recognises those posting URLs while excluding
...
_JOB_DETAIL_PREFIX = "job-detail"


class VincereAdapter(CareerSourceAdapter):
    """Fetch jobs from public Vincere careers site pages."""

    adapter_name = "vincere"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Vincere jobs.

        Args:
            base_url: Vincere careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Vincere careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Vincere posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Vincere job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Vincere URL.

        Args:
            job_url: Vincere job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _JOB_DETAIL_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Vincere posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Vincere job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Vincere anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Vincere anchor metadata or markup."""
