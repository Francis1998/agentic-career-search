"""SilkRoad Recruiting public careers site adapter.

SilkRoad Recruiting-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-144. This adapter recognises those posting URLs while excluding
...
_JOBS_PREFIX = "jobs"
_JOB_PREFIX = "job"
_POSTING_PREFIX = "posting"
_POSTINGS_PREFIX = "postings"
_OPPORTUNITY_PREFIX = "opportunity"


class SilkRoadAdapter(CareerSourceAdapter):
    """Fetch jobs from public SilkRoad Recruiting careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse SilkRoad Recruiting jobs.

        Args:
            base_url: SilkRoad Recruiting careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse SilkRoad Recruiting careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a SilkRoad Recruiting posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a SilkRoad Recruiting URL."""
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _JOB_PREFIX and index + 1 < len(parts):
                ...
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
            if part == _POSTING_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
            if part == _POSTINGS_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
            if part == _OPPORTUNITY_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid SilkRoad Recruiting posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a SilkRoad Recruiting job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a SilkRoad Recruiting anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from SilkRoad Recruiting anchor metadata or markup."""
