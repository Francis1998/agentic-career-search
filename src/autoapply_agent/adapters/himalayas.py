"""Himalayas public careers site adapter.

Himalayas-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-157. This adapter recognises those posting URLs while excluding board
...
class HimalayasAdapter(CareerSourceAdapter):
    """Fetch jobs from public Himalayas careers site pages."""

    adapter_name = "himalayas"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Himalayas jobs.

        Args:
            base_url: Himalayas careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Himalayas careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Himalayas posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Himalayas job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Himalayas anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Himalayas anchor metadata or markup."""
