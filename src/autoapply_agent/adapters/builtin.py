"""Built In public careers site adapter.

Built In-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-152. This adapter recognises those posting URLs while excluding board
indexes plus apply/login/about links.
"""
...
_JOB_PREFIX = "job"
_JOBS_PREFIX = "jobs"
_COMPANY_JOBS_PREFIX = "company-jobs"
...
class BuiltinAdapter(CareerSourceAdapter):
    """Fetch jobs from public Built In careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Built In jobs.

        Args:
            base_url: Built In careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Built In careers HTML into job candidates.
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        ...
        for index, part in enumerate(lowered):
            if part == "careers" and index + 2 < len(parts) and lowered[index + 1] == "job":
                candidate = parts[index + 2]
                if cls._is_job_id(candidate):
                    if (
                        index + 3 == len(lowered)
                        or lowered[index + 3] not in _NON_POSTING_TERMINALS
                    ):
                        return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Built In posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Built In job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Built In anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Built In anchor metadata or markup."""
