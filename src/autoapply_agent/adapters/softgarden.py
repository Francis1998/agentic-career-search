"""Softgarden public careers site adapter.

Softgarden-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/job/{id}``, ``/jobs/{id}``,
``/vacancies/{id}``, ``/vacancy/{id}``, and ``/position/{id}``. This adapter
...
_JOBS_PREFIX = "jobs"
_JOB_PREFIX = "job"
_VACANCIES_PREFIX = "vacancies"
_VACANCY_PREFIX = "vacancy"
_POSITION_PREFIX = "position"


class SoftgardenAdapter(CareerSourceAdapter):
    """Fetch jobs from public Softgarden careers site pages."""

    adapter_name = "softgarden"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Softgarden jobs.

        Args:
            base_url: Softgarden careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Softgarden careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Softgarden posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Softgarden job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Softgarden URL.

        Args:
            job_url: Softgarden job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _VACANCIES_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate

            if part == _VACANCY_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Softgarden posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Softgarden job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Softgarden anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Softgarden anchor metadata or markup."""
