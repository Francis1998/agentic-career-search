"""TheMuse public careers site adapter.

TheMuse public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-185. This adapter recognises those posting URLs while excluding board
...
class ThemuseAdapter(CareerSourceAdapter):
    """Fetch jobs from public TheMuse careers site pages."""

    adapter_name = "themuse"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse TheMuse jobs.

        Args:
            base_url: TheMuse careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse TheMuse careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid TheMuse posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a TheMuse job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a TheMuse anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from TheMuse anchor metadata or markup."""
