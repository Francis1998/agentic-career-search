"""Crypto Jobs public careers site adapter.

Crypto Jobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-175. This adapter recognises those posting URLs while excluding board
...
_CRYPTO_JOBS_PREFIX = "crypto-jobs"
_WEB3_PREFIX = "web3"
...
class CryptojobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public Crypto Jobs careers site pages."""

    adapter_name = "cryptojobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Crypto Jobs jobs.

        Args:
            base_url: Crypto Jobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Crypto Jobs careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Crypto Jobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Crypto Jobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Crypto Jobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Crypto Jobs anchor metadata or markup."""
