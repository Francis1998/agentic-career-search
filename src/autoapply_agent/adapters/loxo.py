"""Loxo public careers site adapter.

Loxo-hosted public careers boards expose listing pages whose postings render as
anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/positions/{id}``, ``/careers/{id}``, and ``/careers/job/{id}``. This adapter
...
class LoxoAdapter(CareerSourceAdapter):
    """Fetch jobs from public Loxo careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Loxo jobs.

        Args:
            base_url: Loxo careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Loxo careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Loxo posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Loxo job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Loxo URL.

        Args:
            job_url: Loxo job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Loxo posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Loxo job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Loxo anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Loxo anchor metadata or markup."""
