"""Cornerstone public careers site adapter.

Cornerstone-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/opening/{id}``. This adapter recognises those posting URLs while excluding
...
class CornerstoneAdapter(CareerSourceAdapter):
    """Fetch jobs from public Cornerstone careers site pages."""

    adapter_name = "cornerstone"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Cornerstone jobs.

        Args:
            base_url: Cornerstone careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Cornerstone careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Cornerstone posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Cornerstone job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Cornerstone URL.

        Args:
            job_url: Cornerstone job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Cornerstone posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Cornerstone job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Cornerstone anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Cornerstone anchor metadata or markup."""
