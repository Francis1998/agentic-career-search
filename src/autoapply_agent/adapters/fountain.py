"""Fountain public careers site adapter.

Fountain (``{org}.fountain.com`` / ``web.fountain.com``) hosts public careers
boards whose listing pages render each posting as an anchor. Detail hrefs commonly
follow ``/apply/{company}/{positionId}``, ``/apply/{slug}`` on tenant subdomains,
or ``/jobs/{jobId}``, ``/openings/{id}``, and ``/positions/{id}``. This adapter
recognises those posting URLs while excluding the board index, apply confirmation,
login steps, and navigation links.
"""
...
class FountainAdapter(CareerSourceAdapter):
    """Fetch jobs from public Fountain careers site pages."""

    adapter_name = "fountain"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Fountain jobs.

        Args:
            base_url: Fountain careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Fountain careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Fountain posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Fountain apply or position
            detail shape.
        """
    ...
    def _extract_external_id(job_url: str | None) -> str | None:
        """Extract the posting id from a Fountain URL.

        Args:
            job_url: Fountain job URL or href.

        Returns:
            Position or job id when a recognised detail shape is present.
        """
        ...
        for index, part in enumerate(parts):
            if part == "apply" and index + 1 < len(parts):
                if index + 2 < len(parts):
                    ...
                    if index + 3 < len(parts):
                        terminal = parts[index + 3].lower()
                        if terminal in _NON_POSTING_TERMINALS:
                            continue
                    return candidate
                if host == "web.fountain.com":
                    continue
            ...
            if part in {"jobs", "openings", "positions"} and index + 1 < len(parts):
                candidate = parts[index + 1]
                if not _JOB_ID_SEGMENT.match(candidate):
                    continue
                if candidate.lower() in _NON_POSTING_TERMINALS:
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Fountain anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Fountain anchor metadata or markup."""
