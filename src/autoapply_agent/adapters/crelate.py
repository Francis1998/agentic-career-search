"""Crelate public careers site adapter.

Crelate-hosted public careers boards expose listing pages whose postings
...
class CrelateAdapter(CareerSourceAdapter):
    """Fetch jobs from public Crelate careers site pages."""

    adapter_name = "crelate"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Crelate jobs.

        Args:
            base_url: Crelate careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Crelate careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Crelate posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Crelate job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Crelate URL.

        Args:
            job_url: Crelate job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Crelate posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Crelate job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Crelate anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Crelate anchor metadata or markup."""
