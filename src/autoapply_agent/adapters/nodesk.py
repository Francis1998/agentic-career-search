"""NoDesk public careers site adapter.

NoDesk public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-160. This adapter recognises those posting URLs while excluding board
...
class NodeskAdapter(CareerSourceAdapter):
    """Fetch jobs from public NoDesk careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse NoDesk jobs.

        Args:
            base_url: NoDesk careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse NoDesk careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid NoDesk posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a NoDesk job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a NoDesk anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from NoDesk anchor metadata or markup."""
