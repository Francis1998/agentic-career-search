"""JobDiva public careers site adapter.

JobDiva-hosted public careers boards expose listing pages whose postings
...
class JobDivaAdapter(CareerSourceAdapter):
    """Fetch jobs from public JobDiva careers site pages."""

    adapter_name = "jobdiva"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse JobDiva jobs.

        Args:
            base_url: JobDiva careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse JobDiva careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a JobDiva posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised JobDiva job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a JobDiva URL.

        Args:
            job_url: JobDiva job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _JD_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid JobDiva posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a JobDiva job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a JobDiva anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from JobDiva anchor metadata or markup."""
