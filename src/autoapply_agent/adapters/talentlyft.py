"""TalentLyft public careers site adapter.

TalentLyft-hosted public careers boards expose listing pages whose postings
...
class TalentLyftAdapter(CareerSourceAdapter):
    """Fetch jobs from public TalentLyft careers site pages."""

    adapter_name = "talentlyft"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse TalentLyft jobs.

        Args:
            base_url: TalentLyft careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse TalentLyft careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a TalentLyft posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised TalentLyft job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a TalentLyft URL.

        Args:
            job_url: TalentLyft job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid TalentLyft posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a TalentLyft job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a TalentLyft anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from TalentLyft anchor metadata or markup."""
