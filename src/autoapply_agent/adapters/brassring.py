"""IBM Kenexa BrassRing public careers site adapter.

BrassRing-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/jobdetail/{id}``, ``/FgJobDetail/{id}``, and
``/careers/{id}``. This adapter recognises those posting URLs while excluding
...
_JOBDETAIL_PREFIX = "jobdetail"
_FGJOBDETAIL_PREFIX = "fgjobdetail"


class BrassringAdapter(CareerSourceAdapter):
    """Fetch jobs from public BrassRing careers site pages."""

    adapter_name = "brassring"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse BrassRing jobs.

        Args:
            base_url: BrassRing careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse BrassRing careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a BrassRing posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised BrassRing job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a BrassRing URL.

        Args:
            job_url: BrassRing job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _JOBDETAIL_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate

            if part == _FGJOBDETAIL_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid BrassRing posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a BrassRing job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a BrassRing anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from BrassRing anchor metadata or markup."""
