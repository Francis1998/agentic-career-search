"""ApplicantPro public careers site adapter.

ApplicantPro-hosted public careers boards expose listing pages whose postings
...
class ApplicantProAdapter(CareerSourceAdapter):
    """Fetch jobs from public ApplicantPro careers site pages."""

    adapter_name = "applicantpro"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse ApplicantPro jobs.

        Args:
            base_url: ApplicantPro careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse ApplicantPro careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at an ApplicantPro posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised ApplicantPro job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from an ApplicantPro URL.

        Args:
            job_url: ApplicantPro job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid ApplicantPro posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like an ApplicantPro job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from an ApplicantPro anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from ApplicantPro anchor metadata or markup."""
