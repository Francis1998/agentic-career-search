"""Levels.fyi public careers site adapter.

Levels.fyi public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-178. This adapter recognises those posting URLs while excluding board
...
class LevelsfyiAdapter(CareerSourceAdapter):
    """Fetch jobs from public Levels.fyi careers site pages."""

    adapter_name = "levelsfyi"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Levels.fyi jobs.

        Args:
            base_url: Levels.fyi careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Levels.fyi careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Levels.fyi posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Levels.fyi job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Levels.fyi anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Levels.fyi anchor metadata or markup."""
