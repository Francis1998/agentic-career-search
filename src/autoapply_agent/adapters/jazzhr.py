"""JazzHR public careers portal adapter.

JazzHR powers public careers boards commonly hosted on
``{tenant}.applytojob.com/apply``. Posting detail links expose a job id via
``/apply/{jobId}`` or ``/apply/{jobId}/{slug}`` paths. Non-posting navigation
links are excluded. This adapter mirrors the SuccessFactors/Taleo/Zoho Recruit
URL-shape scrapers used for other ATS sources.
"""
...
_NON_JOB_ID_SEGMENTS = frozenset({"apply", "job", "jobs", "login", "search", "signin", "sign-in"})


class JazzHrAdapter(CareerSourceAdapter):
    """Fetch jobs from public JazzHR careers portals."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse JazzHR jobs.

        Args:
            base_url: JazzHR careers portal URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse JazzHR careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a JazzHR posting detail page.

        Args:
            href: Candidate href value.

        Returns:
            True when a job id can be extracted from a JazzHR apply path.
        """
    ...
    def _extract_external_id(job_url: str | None) -> str | None:
        """Extract the job id from a JazzHR careers URL.

        Recognises ``/apply/{jobId}`` and ``/apply/{jobId}/{slug}`` paths.
        Other apply-root links or deeper application-step URLs are rejected.

        Args:
            job_url: JazzHR job URL or href.
        ...
        if len(parts) not in {2, 3} or parts[0].lower() != "apply":
            return None
        candidate = parts[1].strip()
        if candidate.lower() in _NON_JOB_ID_SEGMENTS:
            return None
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a JazzHR anchor.
