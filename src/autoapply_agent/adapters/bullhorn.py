"""Bullhorn public careers site adapter.

Bullhorn-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/Job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/position/{id}``. This adapter
recognises those posting URLs while excluding board indexes plus
apply/login/about links.
"""
...
_CONTAINER_CLASS_PATTERN = re.compile(
    "job|position|posting|opening|role|vacancy|opportunity", re.IGNORECASE
)
...
class BullhornAdapter(CareerSourceAdapter):
    """Fetch jobs from public Bullhorn careers site pages."""

    adapter_name = "bullhorn"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Bullhorn jobs.

        Args:
            base_url: Bullhorn careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Bullhorn careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Bullhorn posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Bullhorn job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Bullhorn URL.

        Args:
            job_url: Bullhorn job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Bullhorn posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Bullhorn job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in {_JOBS_PREFIX, _JOB_PREFIX, _CAREERS_PREFIX, _POSITION_PREFIX}:
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Bullhorn anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Bullhorn anchor metadata or markup."""
