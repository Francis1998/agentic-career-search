"""JustRemote public careers site adapter.

JustRemote public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-159. This adapter recognises those posting URLs while excluding board
...
class JustremoteAdapter(CareerSourceAdapter):
    """Fetch jobs from public JustRemote careers site pages."""

    adapter_name = "justremote"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse JustRemote jobs.

        Args:
            base_url: JustRemote careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse JustRemote careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid JustRemote posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a JustRemote job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a JustRemote anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from JustRemote anchor metadata or markup."""
