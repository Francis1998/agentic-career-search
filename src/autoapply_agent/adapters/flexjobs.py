"""FlexJobs public careers site adapter.

FlexJobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-165. This adapter recognises those posting URLs while excluding board
...
class FlexjobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public FlexJobs careers site pages."""

    adapter_name = "flexjobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse FlexJobs jobs.

        Args:
            base_url: FlexJobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse FlexJobs careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid FlexJobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a FlexJobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a FlexJobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from FlexJobs anchor metadata or markup."""
