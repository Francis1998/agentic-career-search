"""PCRecruiter public careers site adapter.

PCRecruiter-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/requisition/{id}``. This adapter recognises those posting URLs while excluding
...
_REQUISITION_PREFIX = "requisition"


class PCRecruiterAdapter(CareerSourceAdapter):
    """Fetch jobs from public PCRecruiter careers site pages."""

    adapter_name = "pcrecruiter"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse PCRecruiter jobs.

        Args:
            base_url: PCRecruiter careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse PCRecruiter careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a PCRecruiter posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised PCRecruiter job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a PCRecruiter URL.

        Args:
            job_url: PCRecruiter job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _REQUISITION_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid PCRecruiter posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a PCRecruiter job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a PCRecruiter anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from PCRecruiter anchor metadata or markup."""
