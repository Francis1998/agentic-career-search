"""Paycom public careers site adapter.

Paycom-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-148. This adapter recognises those posting URLs while excluding board
...
class PaycomAdapter(CareerSourceAdapter):
    """Fetch jobs from public Paycom careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Paycom jobs.

        Args:
            base_url: Paycom careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Paycom careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Paycom posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Paycom job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Paycom anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Paycom anchor metadata or markup."""
