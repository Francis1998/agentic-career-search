"""EuroTechJobs public careers site adapter.

EuroTechJobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-167. This adapter recognises those posting URLs while excluding board
...
class EurotechjobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public EuroTechJobs careers site pages."""

    adapter_name = "eurotechjobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse EuroTechJobs jobs.

        Args:
            base_url: EuroTechJobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse EuroTechJobs careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid EuroTechJobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a EuroTechJobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a EuroTechJobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from EuroTechJobs anchor metadata or markup."""
