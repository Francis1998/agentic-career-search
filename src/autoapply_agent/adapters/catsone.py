"""CATS public careers site adapter.

CATS-hosted public careers boards expose listing pages whose postings render
as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/postings/{id}``. This adapter
...
class CatsoneAdapter(CareerSourceAdapter):
    """Fetch jobs from public CATS careers site pages."""

    adapter_name = "catsone"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse CATS jobs.

        Args:
            base_url: CATS careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse CATS careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid CATS posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a CATS job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a CATS anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from CATS anchor metadata or markup."""
