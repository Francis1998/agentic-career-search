"""HireEZ / Hiretual public careers site adapter.

HireEZ-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/positions/{id}``. This adapter recognises those posting URLs while excluding
...
class HireezAdapter(CareerSourceAdapter):
    """Fetch jobs from public HireEZ careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse HireEZ jobs.

        Args:
            base_url: HireEZ careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse HireEZ careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a HireEZ posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised HireEZ job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a HireEZ URL.

        Args:
            job_url: HireEZ job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid HireEZ posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a HireEZ job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a HireEZ anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from HireEZ anchor metadata or markup."""
