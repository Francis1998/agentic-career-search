"""Python Jobs public careers site adapter.

Python Jobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-180. This adapter recognises those posting URLs while excluding board
...
_PYTHON_JOBS_PREFIX = "python-jobs"
...
class PythonjobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public Python Jobs careers site pages."""

    adapter_name = "pythonjobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Python Jobs jobs.

        Args:
            base_url: Python Jobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Python Jobs careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Python Jobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Python Jobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Python Jobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Python Jobs anchor metadata or markup."""
