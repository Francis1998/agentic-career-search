"""Welcome to the Jungle (WTTJ) public careers site adapter.

Welcome to the Jungle public recruiting boards expose listing pages whose
postings render as anchors. Detail hrefs commonly follow the shapes documented
in ADR-155. This adapter recognises those posting URLs while excluding board
indexes plus apply/login/about/index links.
"""
...
_OFFERS_PREFIX = "offers"
_OFFER_PREFIX = "offer"
_DETAIL_PREFIXES = frozenset(
    {
        _JOBS_PREFIX,
        _JOB_PREFIX,
        _COMPANIES_PREFIX,
...
class WelcometothejungleAdapter(CareerSourceAdapter):
    """Fetch jobs from public Welcome to the Jungle careers site pages."""

    adapter_name = "welcometothejungle"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Welcome to the Jungle jobs.

        Args:
            base_url: Welcome to the Jungle careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Welcome to the Jungle careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Welcome to the Jungle posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Welcome to the Jungle URL."""
        ...
        for index, part in enumerate(lowered):
            if (
                ...
                if cls._is_job_id(candidate):
                    if (
                        index + 4 == len(lowered)
                        or lowered[index + 4] not in _NON_POSTING_TERMINALS
                    ):
                        return candidate
        ...
        for index, part in enumerate(lowered):
            if part not in _DETAIL_PREFIXES or index + 1 >= len(parts):
                continue
            # Skip bare /companies/{slug} — that is a company page, not a job id.
            if part == _COMPANIES_PREFIX:
                continue
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid WTTJ posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a WTTJ job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in _DETAIL_PREFIXES:
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Welcome to the Jungle anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from WTTJ anchor metadata or markup."""
