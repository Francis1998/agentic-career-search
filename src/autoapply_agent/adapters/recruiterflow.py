"""Recruiterflow public careers site adapter.

Recruiterflow-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/job/{id}``, ``/openings/{id}``, and ``/opening/{id}``. This adapter
recognises those posting URLs while excluding board indexes plus apply/login/about
links.
"""
...
_OPENING_PREFIXES = frozenset({"opening", "openings"})


class RecruiterflowAdapter(CareerSourceAdapter):
    """Fetch jobs from public Recruiterflow careers site pages."""

    adapter_name = "recruiterflow"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Recruiterflow jobs.

        Args:
            base_url: Recruiterflow careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Recruiterflow careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Recruiterflow posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Recruiterflow job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Recruiterflow URL.

        Args:
            job_url: Recruiterflow job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part in _OPENING_PREFIXES and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Recruiterflow posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Recruiterflow job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in {_JOBS_PREFIX, _JOB_PREFIX, _CAREERS_PREFIX} | _OPENING_PREFIXES:
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Recruiterflow anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Recruiterflow anchor metadata or markup."""
