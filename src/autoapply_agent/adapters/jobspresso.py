"""Jobspresso public careers site adapter.

Jobspresso public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-161. This adapter recognises those posting URLs while excluding board
...
class JobspressoAdapter(CareerSourceAdapter):
    """Fetch jobs from public Jobspresso careers site pages."""

    adapter_name = "jobspresso"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Jobspresso jobs.

        Args:
            base_url: Jobspresso careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Jobspresso careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Jobspresso posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Jobspresso job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Jobspresso anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Jobspresso anchor metadata or markup."""
