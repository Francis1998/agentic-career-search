"""Paylocity public careers site adapter.

Paylocity-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``,
``/JobDetails/{id}``, ``/careers/{id}``, ``/careers/job/{id}``, and
``/openings/{id}``. This adapter recognises those posting URLs while excluding
...
_JOBS_PREFIX = "jobs"
_JOB_PREFIX = "job"
_JOBDETAILS_PREFIX = "jobdetails"
_CAREERS_PREFIX = "careers"
_OPENINGS_PREFIX = "openings"


class PaylocityAdapter(CareerSourceAdapter):
    """Fetch jobs from public Paylocity careers site pages."""

    adapter_name = "paylocity"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Paylocity jobs.

        Args:
            base_url: Paylocity careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Paylocity careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Paylocity posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Paylocity job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Paylocity URL.

        Args:
            job_url: Paylocity job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _JOBDETAILS_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate

            if part == _OPENINGS_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Paylocity posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Paylocity job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Paylocity anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Paylocity anchor metadata or markup."""
