"""Jibe public careers site adapter.

Jibe-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-146. This adapter recognises those posting URLs while excluding
...
class JibeAdapter(CareerSourceAdapter):
    """Fetch jobs from public Jibe careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Jibe jobs.

        Args:
            base_url: Jibe careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Jibe careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Jibe posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Jibe job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Jibe anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Jibe anchor metadata or markup."""
