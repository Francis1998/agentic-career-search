"""RemoteOK public careers site adapter.

RemoteOK-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-153. This adapter recognises those posting URLs while excluding board
...
class RemoteokAdapter(CareerSourceAdapter):
    """Fetch jobs from public RemoteOK careers site pages."""

    adapter_name = "remoteok"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse RemoteOK jobs.

        Args:
            base_url: RemoteOK careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse RemoteOK careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid RemoteOK posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a RemoteOK job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a RemoteOK anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from RemoteOK anchor metadata or markup."""
