"""PowerToFly public careers site adapter.

PowerToFly public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-173. This adapter recognises those posting URLs while excluding board
...
class PowertoflyAdapter(CareerSourceAdapter):
    """Fetch jobs from public PowerToFly careers site pages."""

    adapter_name = "powertofly"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse PowerToFly jobs.

        Args:
            base_url: PowerToFly careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse PowerToFly careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid PowerToFly posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a PowerToFly job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a PowerToFly anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from PowerToFly anchor metadata or markup."""
