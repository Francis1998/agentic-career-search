"""Naukri public careers site adapter.

Naukri public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-193. This adapter recognises those posting URLs while excluding board
...
_JOB_LISTINGS_PREFIX = "job-listings"
_JOBDETAIL_PREFIX = "jobdetail"
_JOBS_PREFIX = "jobs"
_JOB_DESCRIPTION_PREFIX = "job-description"
_DETAIL_PREFIXES = frozenset(
    {
        _JOB_LISTINGS_PREFIX,
        _JOBDETAIL_PREFIX,
        _JOBS_PREFIX,
        _JOB_DESCRIPTION_PREFIX,
    }
)
_COMPOUND_PREFIXES = (("recruiters", "job"),)


class NaukriAdapter(CareerSourceAdapter):
    """Fetch jobs from public Naukri careers site pages."""
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        ...
        for index in range(len(lowered) - 2):
            ...
            if pair not in _COMPOUND_PREFIXES:
                continue
    ...
    def _is_job_id(candidate: str) -> bool:
        ...
        if candidate.lower() in _DETAIL_PREFIXES:
            return False
        if candidate.lower() in {first for first, _second in _COMPOUND_PREFIXES}:
            return False
        if candidate.lower() in {second for _first, second in _COMPOUND_PREFIXES}:
            return False
