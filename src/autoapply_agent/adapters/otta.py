"""Otta public careers site adapter.

Otta-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-151. This adapter recognises those posting URLs while excluding board
...
class OttaAdapter(CareerSourceAdapter):
    """Fetch jobs from public Otta careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Otta jobs.

        Args:
            base_url: Otta careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Otta careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Otta posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Otta job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Otta anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Otta anchor metadata or markup."""
