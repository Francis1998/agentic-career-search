"""CareerPlug public careers site adapter.

CareerPlug-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/listings/{id}``. This adapter recognises those posting URLs while excluding
...
class CareerPlugAdapter(CareerSourceAdapter):
    """Fetch jobs from public CareerPlug careers site pages."""

    adapter_name = "careerplug"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse CareerPlug jobs.

        Args:
            base_url: CareerPlug careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse CareerPlug careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a CareerPlug posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised CareerPlug job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a CareerPlug URL.

        Args:
            job_url: CareerPlug job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _LISTINGS_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid CareerPlug posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a CareerPlug job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a CareerPlug anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from CareerPlug anchor metadata or markup."""
