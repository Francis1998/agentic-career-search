"""Wellfound public careers site adapter.

Wellfound-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-150. This adapter recognises those posting URLs while excluding board
...
_ROLE_PREFIX = "role"
_ROLES_PREFIX = "roles"
_STARTUP_JOBS_PREFIX = "startup-jobs"
_DETAIL_PREFIXES = frozenset(
    {
        ...
        _ROLE_PREFIX,
        _ROLES_PREFIX,
        _STARTUP_JOBS_PREFIX,
    }
)


class WellfoundAdapter(CareerSourceAdapter):
    """Fetch jobs from public Wellfound careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Wellfound jobs.

        Args:
            base_url: Wellfound careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Wellfound careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Wellfound posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Wellfound job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Wellfound anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Wellfound anchor metadata or markup."""
