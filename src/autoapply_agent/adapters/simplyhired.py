"""SimplyHired public careers site adapter.

SimplyHired public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-181. This adapter recognises those posting URLs while excluding board
...
class SimplyhiredAdapter(CareerSourceAdapter):
    """Fetch jobs from public SimplyHired careers site pages."""

    adapter_name = "simplyhired"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse SimplyHired jobs.

        Args:
            base_url: SimplyHired careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse SimplyHired careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid SimplyHired posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a SimplyHired job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a SimplyHired anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from SimplyHired anchor metadata or markup."""
