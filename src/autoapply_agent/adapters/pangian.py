"""Pangian public careers site adapter.

Pangian public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-169. This adapter recognises those posting URLs while excluding board
...
class PangianAdapter(CareerSourceAdapter):
    """Fetch jobs from public Pangian careers site pages."""

    adapter_name = "pangian"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Pangian jobs.

        Args:
            base_url: Pangian careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Pangian careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Pangian posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Pangian job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Pangian anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Pangian anchor metadata or markup."""
