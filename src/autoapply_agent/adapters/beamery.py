"""Beamery public careers site adapter.

Beamery-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/campaign/{id}``. This adapter recognises those posting URLs while excluding
...
class BeameryAdapter(CareerSourceAdapter):
    """Fetch jobs from public Beamery careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Beamery jobs.

        Args:
            base_url: Beamery careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Beamery careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Beamery posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Beamery job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Beamery URL.

        Args:
            job_url: Beamery job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _CAMPAIGN_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Beamery posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Beamery job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Beamery anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Beamery anchor metadata or markup."""
