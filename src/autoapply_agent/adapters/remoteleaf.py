"""RemoteLeaf public careers site adapter.

RemoteLeaf public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-170. This adapter recognises those posting URLs while excluding board
...
class RemoteleafAdapter(CareerSourceAdapter):
    """Fetch jobs from public RemoteLeaf careers site pages."""

    adapter_name = "remoteleaf"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse RemoteLeaf jobs.

        Args:
            base_url: RemoteLeaf careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse RemoteLeaf careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid RemoteLeaf posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a RemoteLeaf job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a RemoteLeaf anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from RemoteLeaf anchor metadata or markup."""
