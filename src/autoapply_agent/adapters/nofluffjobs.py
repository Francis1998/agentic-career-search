"""No Fluff Jobs public careers site adapter.

No Fluff Jobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-176. This adapter recognises those posting URLs while excluding board
...
class NofluffjobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public No Fluff Jobs careers site pages."""

    adapter_name = "nofluffjobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse No Fluff Jobs jobs.

        Args:
            base_url: No Fluff Jobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse No Fluff Jobs careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a No Fluff Jobs posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a No Fluff Jobs URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid No Fluff Jobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a No Fluff Jobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a No Fluff Jobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from No Fluff Jobs anchor metadata or markup."""
