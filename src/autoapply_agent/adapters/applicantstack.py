"""ApplicantStack public careers site adapter.

ApplicantStack-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/postings/{id}``. This adapter recognises those posting URLs while excluding
...
class ApplicantStackAdapter(CareerSourceAdapter):
    """Fetch jobs from public ApplicantStack careers site pages."""

    adapter_name = "applicantstack"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse ApplicantStack jobs.

        Args:
            base_url: ApplicantStack careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse ApplicantStack careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at an ApplicantStack posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised ApplicantStack job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from an ApplicantStack URL.

        Args:
            job_url: ApplicantStack job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid ApplicantStack posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like an ApplicantStack job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from an ApplicantStack anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from ApplicantStack anchor metadata or markup."""
