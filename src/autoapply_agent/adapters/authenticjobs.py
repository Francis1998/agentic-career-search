"""Authentic Jobs public careers site adapter.

Authentic Jobs public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-166. This adapter recognises those posting URLs while excluding board
...
class AuthenticjobsAdapter(CareerSourceAdapter):
    """Fetch jobs from public Authentic Jobs careers site pages."""

    adapter_name = "authenticjobs"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Authentic Jobs jobs.

        Args:
            base_url: Authentic Jobs careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Authentic Jobs careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Authentic Jobs posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Authentic Jobs URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Authentic Jobs posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Authentic Jobs job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Authentic Jobs anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Authentic Jobs anchor metadata or markup."""
