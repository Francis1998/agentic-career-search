"""Work at a Startup public careers site adapter.

Work at a Startup public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-174. This adapter recognises those posting URLs while excluding board
...
class WorkatastartupAdapter(CareerSourceAdapter):
    """Fetch jobs from public Work at a Startup careers site pages."""

    adapter_name = "workatastartup"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Work at a Startup jobs.

        Args:
            base_url: Work at a Startup careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Work at a Startup careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Work at a Startup posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Work at a Startup URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Work at a Startup posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Work at a Startup job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Work at a Startup anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Work at a Startup anchor metadata or markup."""
