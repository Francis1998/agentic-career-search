"""Working Nomads public careers site adapter.

Working Nomads public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-158. This adapter recognises those posting URLs while excluding board
...
class WorkingnomadsAdapter(CareerSourceAdapter):
    """Fetch jobs from public Working Nomads careers site pages."""

    adapter_name = "workingnomads"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Working Nomads jobs.

        Args:
            base_url: Working Nomads careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Working Nomads careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Working Nomads posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Working Nomads URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Working Nomads posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Working Nomads job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Working Nomads anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Working Nomads anchor metadata or markup."""
