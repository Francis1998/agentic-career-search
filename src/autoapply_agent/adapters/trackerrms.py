"""TrackerRMS public careers site adapter.

TrackerRMS-hosted public careers boards expose listing pages whose postings
...
class TrackerRmsAdapter(CareerSourceAdapter):
    """Fetch jobs from public TrackerRMS careers site pages."""

    adapter_name = "trackerrms"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse TrackerRMS jobs.

        Args:
            base_url: TrackerRMS careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse TrackerRMS careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a TrackerRMS posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised TrackerRMS job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a TrackerRMS URL.

        Args:
            job_url: TrackerRMS job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid TrackerRMS posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a TrackerRMS job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a TrackerRMS anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from TrackerRMS anchor metadata or markup."""
