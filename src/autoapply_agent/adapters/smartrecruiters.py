"""SmartRecruiters public careers site adapter.
...
class SmartRecruitersAdapter(CareerSourceAdapter):
    """Fetch jobs from public SmartRecruiters careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse SmartRecruiters jobs.

        Args:
            base_url: SmartRecruiters careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse SmartRecruiters careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a SmartRecruiters posting.
        ...
        Returns:
            True when the URL exposes a numeric ``jobId`` path segment.
        """
    ...
    def _extract_external_id(job_url: str | None) -> str | None:
        """Extract the posting job id from a SmartRecruiters URL.

        Args:
            job_url: SmartRecruiters job URL or href.

        Returns:
            Numeric job id string when the ``{jobId}-{slug}`` shape is present.
        """
        ...
        for part in parts:
            match = _JOB_ID_PATTERN.match(part)
            if match:
    ...
    def _extract_location(anchor: object) -> str | None:
        ...
        SmartRecruiters groups a posting's metadata (department, location)
        alongside the title anchor. The location is looked up within the
        anchor's nearest posting container so a posting without its own location
        does not inherit a sibling's location.
        ...
        if find_parent is None:
            return None
        container = find_parent(attrs={"class": re.compile("opening|job|posting", re.IGNORECASE)})
