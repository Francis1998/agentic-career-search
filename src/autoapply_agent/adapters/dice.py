"""Dice public careers site adapter.

Dice public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-177. This adapter recognises those posting URLs while excluding board
...
class DiceAdapter(CareerSourceAdapter):
    """Fetch jobs from public Dice careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Dice jobs.

        Args:
            base_url: Dice careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Dice careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Dice posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Dice job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Dice anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Dice anchor metadata or markup."""
