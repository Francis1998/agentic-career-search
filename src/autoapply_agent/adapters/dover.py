"""Dover public careers site adapter.

Dover-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/openings/{id}``. This
...
class DoverAdapter(CareerSourceAdapter):
    """Fetch jobs from public Dover careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Dover jobs.

        Args:
            base_url: Dover careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Dover careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Dover posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Dover job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Dover URL.

        Args:
            job_url: Dover job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Dover posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Dover job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Dover anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Dover anchor metadata or markup."""
