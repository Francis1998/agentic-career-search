"""Avature public careers site adapter.

Avature-hosted public careers portals expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/JobDetail/{id}``, the
legacy ``/JobDetail.aspx?JobId={id}`` query form, ``/careers/{id}``,
``/careers/job/{id}``, ``/careers/VacancyDetail/{id}``, ``/Vacancy/{id}``,
or ``/vacancies/{id}``. This adapter recognises those posting URLs while
excluding board indexes plus apply/login/RegisterCandidate/about links.
"""
...
_JOB_DETAIL_MARKERS = frozenset({"jobdetail", "jobdetail.aspx"})
_VACANCY_MARKERS = frozenset({"vacancy", "vacancies", "vacancydetail"})
_CAREERS_DETAIL_PREFIXES = frozenset({"job", "vacancydetail"})


class AvatureAdapter(CareerSourceAdapter):
    """Fetch jobs from public Avature careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Avature jobs.

        Args:
            base_url: Avature careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Avature careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at an Avature posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Avature job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from an Avature URL.

        Prefers a path-segment id when present (for example
        ``/JobDetail/{id}``), then falls back to ``JobId``-style query
        parameters used by ``/JobDetail.aspx?JobId={id}``.

        Args:
            job_url: Avature job URL or href.
        ...
        path_id = cls._path_external_id(parts, lowered)
        if path_id is not None:
            return path_id

        return cls._query_external_id(parsed.query)

    @classmethod
    def _path_external_id(cls, parts: list[str], lowered: list[str]) -> str | None:
        """Extract a posting id from recognised Avature path shapes."""

        for index, part in enumerate(lowered):
            if part in _JOB_DETAIL_MARKERS:
            ...
            if part == "careers":
                ...
                remainder_lowered = lowered[index + 1 :]
                if len(remainder) == 1:
                    candidate = remainder[0]
                    if cls._is_job_id(candidate):
                        return candidate
                if len(remainder) >= 2 and remainder_lowered[0] in _CAREERS_DETAIL_PREFIXES:
                    candidate = remainder[1]
                    if cls._accepts_detail_candidate(lowered, index + 2, candidate):
                        return candidate

            if part in {"vacancy", "vacancies"}:
                if index + 1 >= len(parts):
                    continue
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
    ...
    @classmethod
    def _query_external_id(cls, query: str) -> str | None:
        """Extract a posting id from Avature query parameters."""

        parsed_query = parse_qs(query)
        lowered_map = {key.lower(): values for key, values in parsed_query.items()}
        for key in ("jobid", "job_id"):
            values = lowered_map.get(key)
            if not values:
                continue
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Avature posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like an Avature job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in _JOB_DETAIL_MARKERS | _VACANCY_MARKERS | {
            "careers",
            "job",
            "jobs",
        }:
            return False
        if "." in candidate:
            # Reject script endpoints such as JobDetail.aspx used as ids.
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from an Avature anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Avature anchor metadata or markup."""
