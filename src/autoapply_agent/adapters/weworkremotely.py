"""WeWorkRemotely public careers site adapter.

WeWorkRemotely-hosted public recruiting boards expose listing pages whose
postings render as anchors. Detail hrefs commonly follow the shapes documented
in ADR-154. This adapter recognises those posting URLs while excluding board
...
_REMOTE_JOBS_PREFIX = "remote-jobs"
_JOBS_PREFIX = "jobs"
_JOB_PREFIX = "job"
_LISTINGS_PREFIX = "listings"
_LISTING_PREFIX = "listing"
...
class WeworkremotelyAdapter(CareerSourceAdapter):
    """Fetch jobs from public WeWorkRemotely careers site pages."""

    adapter_name = "weworkremotely"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse WeWorkRemotely jobs.

        Args:
            base_url: WeWorkRemotely careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse WeWorkRemotely careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a WeWorkRemotely posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a WeWorkRemotely URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid WeWorkRemotely posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a WeWorkRemotely job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a WeWorkRemotely anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from WeWorkRemotely anchor metadata or markup."""
