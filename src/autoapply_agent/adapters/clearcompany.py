"""ClearCompany public careers site adapter.

ClearCompany-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/careers/job/{id}``,
``/careers/{id}``, ``/jobs/{id}``, ``/job/{id}-{slug}``, and
``/position/{id}``. This adapter recognises those posting URLs while excluding
...
_SLUG_ID_SEGMENT = re.compile(r"^(?P<id>[A-Za-z0-9]+)-(?P<slug>.+)$")
...
class ClearCompanyAdapter(CareerSourceAdapter):
    """Fetch jobs from public ClearCompany careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse ClearCompany jobs.

        Args:
            base_url: ClearCompany careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse ClearCompany careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a ClearCompany posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised ClearCompany job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a ClearCompany URL.

        Args:
            job_url: ClearCompany job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if (
                part == _CAREERS_PREFIX
                and index + 2 < len(parts)
                and lowered[index + 1] == _JOB_PREFIX
            ):
            ...
            if part == _JOB_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                posting_id = cls._coerce_slug_id(candidate)
                if posting_id is not None and cls._accepts_detail_candidate(
                    lowered, index + 1, posting_id
                ):
                    return posting_id
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate

            if part == _POSITION_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    @classmethod
    def _coerce_slug_id(cls, segment: str) -> str | None:
        """Resolve a ``{id}-{slug}`` segment to a posting id."""

        slug_match = _SLUG_ID_SEGMENT.match(segment)
        if slug_match and cls._is_job_id(slug_match.group("id")):
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid ClearCompany posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a ClearCompany job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a ClearCompany anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from ClearCompany anchor metadata or markup."""
