"""Tribepad public careers site adapter.

Tribepad-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/vacancy/{id}``. This adapter recognises those posting URLs while excluding
...
class TribepadAdapter(CareerSourceAdapter):
    """Fetch jobs from public Tribepad careers site pages."""

    adapter_name = "tribepad"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Tribepad jobs.

        Args:
            base_url: Tribepad careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Tribepad careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Tribepad posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Tribepad job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Tribepad URL.

        Args:
            job_url: Tribepad job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Tribepad posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Tribepad job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Tribepad anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Tribepad anchor metadata or markup."""
