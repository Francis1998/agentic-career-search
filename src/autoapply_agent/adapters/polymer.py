"""Polymer public careers site adapter.

Polymer-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/positions/{id}``. This
...
class PolymerAdapter(CareerSourceAdapter):
    """Fetch jobs from public Polymer careers site pages."""

    adapter_name = "polymer"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Polymer jobs.

        Args:
            base_url: Polymer careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Polymer careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Polymer posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Polymer job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Polymer URL.

        Args:
            job_url: Polymer job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Polymer posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Polymer job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Polymer anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Polymer anchor metadata or markup."""
