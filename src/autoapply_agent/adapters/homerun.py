"""Homerun public careers site adapter.

Homerun-hosted public careers boards expose listing pages whose postings render
as anchors. Detail hrefs commonly follow ``/jobs/{id}-{slug}``, ``/o/{id}``, and
``/vacancies/{id}``. This adapter recognises those posting URLs while excluding
...
_JOBS_PREFIX = "jobs"
_OPENING_PREFIX = "o"
_VACANCY_PREFIXES = frozenset({"vacancy", "vacancies"})


class HomerunAdapter(CareerSourceAdapter):
    """Fetch jobs from public Homerun careers site pages."""

    adapter_name = "homerun"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Homerun jobs.

        Args:
            base_url: Homerun careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Homerun careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Homerun posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Homerun job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Homerun URL.

        Args:
            job_url: Homerun job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _OPENING_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate

            if part in _VACANCY_PREFIXES and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Homerun posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Homerun job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in {_JOBS_PREFIX, _OPENING_PREFIX} | _VACANCY_PREFIXES:
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Homerun anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Homerun anchor metadata or markup."""
