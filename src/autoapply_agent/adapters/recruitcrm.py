"""RecruitCRM public careers site adapter.

RecruitCRM-hosted public careers boards expose listing pages whose postings
...
class RecruitCrmAdapter(CareerSourceAdapter):
    """Fetch jobs from public RecruitCRM careers site pages."""

    adapter_name = "recruitcrm"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse RecruitCRM jobs.

        Args:
            base_url: RecruitCRM careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse RecruitCRM careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a RecruitCRM posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised RecruitCRM job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a RecruitCRM URL.

        Args:
            job_url: RecruitCRM job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid RecruitCRM posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a RecruitCRM job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a RecruitCRM anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from RecruitCRM anchor metadata or markup."""
