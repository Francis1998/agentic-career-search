"""Remoteco public careers site adapter.

Remoteco public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-163. This adapter recognises those posting URLs while excluding board
...
class RemotecoAdapter(CareerSourceAdapter):
    """Fetch jobs from public Remoteco careers site pages."""

    adapter_name = "remoteco"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Remoteco jobs.

        Args:
            base_url: Remoteco careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Remoteco careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Remoteco posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Remoteco job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Remoteco anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Remoteco anchor metadata or markup."""
