"""Work in the Open public careers site adapter.

Work in the Open public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-179. This adapter recognises those posting URLs while excluding board
...
class WorkintheopenAdapter(CareerSourceAdapter):
    """Fetch jobs from public Work in the Open careers site pages."""

    adapter_name = "workintheopen"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Work in the Open jobs.

        Args:
            base_url: Work in the Open careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Work in the Open careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Work in the Open posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Work in the Open URL."""
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Work in the Open posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Work in the Open job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Work in the Open anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Work in the Open anchor metadata or markup."""
