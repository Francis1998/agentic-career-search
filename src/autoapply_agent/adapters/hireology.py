"""Hireology public careers site adapter.

Hireology-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``,
``/careers/job/{id}``, and ``/job/{id}/{slug}``. This adapter recognises those
posting URLs while excluding board indexes plus apply/login/about links.
"""
...
class HireologyAdapter(CareerSourceAdapter):
    """Fetch jobs from public Hireology careers site pages."""

    adapter_name = "hireology"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Hireology jobs.

        Args:
            base_url: Hireology careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Hireology careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Hireology posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Hireology job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Hireology URL.

        Args:
            job_url: Hireology job URL or href.
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _CAREERS_JOB_PREFIX and index + 2 < len(parts):
                candidate = parts[index + 1]
                slug = parts[index + 2]
                if (
                    cls._accepts_detail_candidate(lowered, index + 1, candidate)
                    and slug.lower() not in _NON_POSTING_TERMINALS
                ):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Hireology posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Hireology job post id."""

        if candidate.lower() in _NON_POSTING_TERMINALS:
            return False
        if candidate.lower() in {_JOBS_PREFIX, _CAREERS_JOB_PREFIX, "careers"}:
            return False
        return bool(_JOB_ID_SEGMENT.match(candidate))

    @classmethod
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Hireology anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Hireology anchor metadata or markup."""
