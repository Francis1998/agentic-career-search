"""Jobylon public careers site adapter.

Jobylon-hosted public careers boards expose listing pages whose postings
...
class JobylonAdapter(CareerSourceAdapter):
    """Fetch jobs from public Jobylon careers site pages."""

    adapter_name = "jobylon"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Jobylon jobs.

        Args:
            base_url: Jobylon careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Jobylon careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Jobylon posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Jobylon job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Jobylon URL.

        Args:
            job_url: Jobylon job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Jobylon posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Jobylon job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Jobylon anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Jobylon anchor metadata or markup."""
