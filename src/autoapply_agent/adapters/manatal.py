"""Manatal public careers site adapter.

Manatal-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/openings/{id}``. This adapter
...
class ManatalAdapter(CareerSourceAdapter):
    """Fetch jobs from public Manatal careers site pages."""

    adapter_name = "manatal"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Manatal jobs.

        Args:
            base_url: Manatal careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Manatal careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Manatal posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Manatal job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Manatal URL.

        Args:
            job_url: Manatal job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Manatal posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Manatal job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Manatal anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Manatal anchor metadata or markup."""
