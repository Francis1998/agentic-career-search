"""ADP Recruiting public careers site adapter.

ADP-hosted public recruiting boards expose listing pages whose postings render
as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/requisitions/{id}``. This
...
_REQUISITIONS_PREFIX = "requisitions"


class AdpAdapter(CareerSourceAdapter):
    """Fetch jobs from public ADP Recruiting careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse ADP Recruiting jobs.

        Args:
            base_url: ADP Recruiting careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse ADP Recruiting careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at an ADP Recruiting posting."""

        return cls._extract_external_id(href) is not None

    @classmethod
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from an ADP Recruiting URL."""
        ...
        for index, part in enumerate(lowered):
            ...
            if part == _REQUISITIONS_PREFIX and index + 1 < len(parts):
                candidate = parts[index + 1]
                if cls._accepts_detail_candidate(lowered, index + 1, candidate):
                    return candidate
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid ADP Recruiting posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like an ADP job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from an ADP Recruiting anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from ADP anchor metadata or markup."""
