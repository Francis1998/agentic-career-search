"""Paradox Olivia public careers site adapter.

Paradox-hosted public careers boards expose listing pages whose postings render
as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/opportunities/{id}``. This
...
class ParadoxAdapter(CareerSourceAdapter):
    """Fetch jobs from public Paradox careers site pages."""

    adapter_name = "paradox"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Paradox jobs.

        Args:
            base_url: Paradox careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Paradox careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Paradox posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Paradox job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Paradox anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Paradox anchor metadata or markup."""
