"""JobAdder public careers site adapter.

JobAdder-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and ``/position/{id}``. This
...
class JobAdderAdapter(CareerSourceAdapter):
    """Fetch jobs from public JobAdder careers site pages."""

    adapter_name = "jobadder"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse JobAdder jobs.

        Args:
            base_url: JobAdder careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse JobAdder careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a JobAdder posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised JobAdder job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a JobAdder URL.

        Args:
            job_url: JobAdder job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid JobAdder posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a JobAdder job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a JobAdder anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from JobAdder anchor metadata or markup."""
