"""Ceipal public careers site adapter.

Ceipal-hosted public careers boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow ``/jobs/{id}``, ``/job/{id}``,
``/careers/{id}``, ``/careers/job/{id}``, and
``/requisitions/{id}``. This adapter recognises those posting URLs while excluding
...
class CeipalAdapter(CareerSourceAdapter):
    """Fetch jobs from public Ceipal careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Ceipal jobs.

        Args:
            base_url: Ceipal careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Ceipal careers HTML into job candidates.
    ...
    def _is_posting_href(cls, href: str | None) -> bool:
        """Report whether an href points at a Ceipal posting.

        Args:
            href: Candidate href value.

        Returns:
            True when the URL exposes a recognised Ceipal job detail shape.
        """
    ...
    def _extract_external_id(cls, job_url: str | None) -> str | None:
        """Extract the posting id from a Ceipal URL.

        Args:
            job_url: Ceipal job URL or href.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Ceipal posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Ceipal job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Ceipal anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Ceipal anchor metadata or markup."""
