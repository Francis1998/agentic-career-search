"""Remotive public careers site adapter.

Remotive-hosted public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-156. This adapter recognises those posting URLs while excluding board
...
class RemotiveAdapter(CareerSourceAdapter):
    """Fetch jobs from public Remotive careers site pages."""

    adapter_name = "remotive"
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Remotive jobs.

        Args:
            base_url: Remotive careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Remotive careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Remotive posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Remotive job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Remotive anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Remotive anchor metadata or markup."""
