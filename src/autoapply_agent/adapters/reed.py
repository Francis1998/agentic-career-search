"""Reed public careers site adapter.

Reed public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-184. This adapter recognises those posting URLs while excluding board
...
class ReedAdapter(CareerSourceAdapter):
    """Fetch jobs from public Reed careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Reed jobs.

        Args:
            base_url: Reed careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Reed careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Reed posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Reed job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Reed anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Reed anchor metadata or markup."""
