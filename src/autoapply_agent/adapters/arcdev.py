"""Arcdev public careers site adapter.

Arcdev public recruiting boards expose listing pages whose postings
render as anchors. Detail hrefs commonly follow the shapes documented in
ADR-164. This adapter recognises those posting URLs while excluding board
...
class ArcdevAdapter(CareerSourceAdapter):
    """Fetch jobs from public Arcdev careers site pages."""
    ...
    async def fetch_jobs(
        ...
        """Fetch and parse Arcdev jobs.

        Args:
            base_url: Arcdev careers site URL.
    ...
    def _parse_html(self, base_url: str, html: str, max_jobs: int) -> list[JobCandidate]:
        """Parse Arcdev careers HTML into job candidates.
    ...
    def _accepts_detail_candidate(
        ...
        """Report whether a path segment is a valid Arcdev posting id."""
    ...
    def _is_job_id(candidate: str) -> bool:
        """Report whether a path segment looks like a Arcdev job post id."""
    ...
    def _anchor_title(cls, anchor: object) -> str | None:
        """Resolve a posting title from a Arcdev anchor."""
    ...
    def _extract_location(anchor: object) -> str | None:
        """Resolve a posting location from Arcdev anchor metadata or markup."""
